...
static void CG_TimerunStatsDown_f(void)
{
	if (!cg.demoPlayback)
	{
		int i = (cg.mvTotalClients > 0) ? (cg.mvCurrentActive->mvInfo & MV_PID) : cg.snap->ps.clientNum;
		...
		if (cg.mvTotalClients < 1 && cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR)
		{
			Pri("You must be a player or following a player to use +timerunstats\n");
			return;
		}

		if (cgs.timerunStats.show == SHOW_SHUTDOWN && cg.time < cgs.timerunStats.fadeTime)
			cgs.timerunStats.fadeTime = 2 * cg.time + STATS_FADE_TIME - cgs.timerunStats.fadeTime;
		else if (cgs.timerunStats.show != SHOW_ON)
			cgs.timerunStats.fadeTime = cg.time + STATS_FADE_TIME;

		cgs.timerunStats.show = SHOW_ON;

		if (cgs.timerunStats.requestTime < cg.time)
		{
			cgs.timerunStats.requestTime = cg.time + 2000;
			trap_SendClientCommand(va("tstats %d", i));
		}
	}
}

static void CG_TimerunStatsUp_f(void)
{
	if (cgs.timerunStats.show == SHOW_ON)
	{
		cgs.timerunStats.show = SHOW_SHUTDOWN;
		if (cg.time < cgs.timerunStats.fadeTime)
			cgs.timerunStats.fadeTime = 2 * cg.time + STATS_FADE_TIME - cgs.timerunStats.fadeTime;
		else
			cgs.timerunStats.fadeTime = cg.time + STATS_FADE_TIME;
	}
...
void CG_StartCamera(const char *name, qboolean startBlack)
{
	...
	//if ( cg.ps.stats[STAT_HEALTH] <= 0 )	// don't allow camera to start if you're dead
	//	return;
...
static void CG_uinfo_block_f(void)
{
	CG_Printf("You are not allowed to view or edit userinfo.\n");
...
static consoleCommand_t	commands[] =
{
	...
	{ "+timerunstats",	CG_TimerunStatsDown_f },
	{ "-timerunstats",	CG_TimerunStatsUp_f },
...
void CG_InitConsoleCommands(void)
{
	...
	trap_AddCommand("m"); // private message

	// tj related cmds
	trap_AddCommand("call"); // = iwant
	trap_AddCommand("dist");
	trap_AddCommand("goback");
	trap_AddCommand("goto");
	trap_AddCommand("iwant");
	trap_AddCommand("load");
	trap_AddCommand("noiwant");
	trap_AddCommand("nocall"); // = noiwant
	trap_AddCommand("nogoto");
	trap_AddCommand("pos");
	trap_AddCommand("save");
	trap_AddCommand("selfrevive");
	trap_AddCommand("resetspeed");
	trap_AddCommand("resettimeruns");
