...
==================
*/
// Dini, tj_drawspeed, drawspeed from etpub. Modified.
...
static float CG_DrawSpeed(float y)
{
	...
	if (thistime > (lasttime + tj_speedinterval.integer))
	{

		if (tj_speedXYonly.integer)
			speed = sqrt(cg.ps.velocity[0] * cg.ps.velocity[0] + cg.ps.velocity[1] * cg.ps.velocity[1]);
		else
			speed = VectorLength(cg.ps.velocity);
	...
	switch (tj_drawspeed.integer)
	{
		default:
		case 1:
			// kw: swapped 1 and 2 to match etpro's b_speedunit
			switch (tj_speedunit.integer)
			{
				default:
				case 0:
					// Units per second
...
static void CG_DrawUpperRight(void)
{
	...
	// Dini, speedometer from etpub
	// forty - speedometer
	if (tj_drawspeed.integer)
	{
		y = CG_DrawSpeed(y);
	}

	// Dini, clock
	if (tj_drawClock.integer)
	{
		y = CG_DrawTime(y);
	}
...
static void CG_DrawBannerPrint(void)
{
	...
	bannerShowTime = tj_bannerTime.integer * 1000;
	...
	sizex = sizey = 0.1f;
	sizex *= tj_bannerSizeX.integer;
	sizey *= tj_bannerSizeY.integer;

	y = tj_bannerY.integer;
	...
	while (1)
	{
		...
		if (tj_bannerShadow.integer)
			CG_Text_Paint_Ext(x, y, sizex, sizey, color, va("^%c%s", colorchar, linebuffer),	0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont2);
		else
			CG_Text_Paint_Ext(x, y, sizex, sizey, color, va("^%c%s", colorchar, linebuffer),	0, 0, ITEM_TEXTSTYLE_NORMAL, &cgs.media.limboFont2);
...
static void CG_DrawMortarReticle(void)
{
	...
	// that means left is cg.ps.viewangles[YAW] - .5f * 180
	angle = 360 - AngleNormalize360(cg.ps.viewangles[YAW] - 90.f);
	...
	// that means left is cg.ps.viewangles[PITCH] - .5f * 50
	angle = AngleNormalize180(360 - (cg.ps.viewangles[PITCH] - 60));
...
static void CG_DrawCrosshair(void)
{
	...
	if (cg.ps.eFlags & EF_PRONE_MOVING)
	{
		return;
	}

	// FIXME: spectators/chasing?
	if (cg.ps.weapon == WP_MORTAR_SET && cg.ps.weaponstate != WEAPON_RAISING)
	{
		CG_DrawMortarReticle();
		return;
	}
...
static void CG_DrawNoShootIcon(void)
{
	...
	if (cg.ps.eFlags &EF_PRONE && cg.snap->ps.weapon == WP_PANZERFAUST)
	{
		trap_R_SetColor(colorRed);
	}
...
static void CG_DrawCrosshairNames(void)
{
	...
	if (!isTank && (tj_hide.integer == 1 && dist < tj_hideDist.integer) || tj_hide.integer == 2)
		return;
...
{
	// Dini, can just as well make this turn this crap off too, I KNOW when I'm spec without having some ugly text telling me.
...
static void CG_DrawVote(void)
{
	...
	// Dini, just an annying feature.. (if there's no fireteam(s) already this shows)
	...
	// Dini, just an annying feature.. (if fireteam(s) exists then this shows..)
...
static void CG_DrawCGazHUD(void)
{
	...
	if (!tj_drawCGaz.integer && !tj_CGazInfo.integer)
		return;

	if (isTimerun.integer && tjg_recordMode.integer)
		return;

	if (ps->persistant[PERS_TEAM] == TEAM_SPECTATOR)
		return;

	a = tj_CGazAlpha.value;
	a = (a > 1.0f) ? 1.0f : (a < 0.0f) ? 0.0f : a;
	color[3] = a;
	y = tj_CGazY.integer > 0 ? tj_CGazY.integer % 480 : 0;
	h = tj_CGazHeight.integer > 0 ? tj_CGazHeight.integer : 0;
	w = tj_CGazWidth.integer > 0 ? tj_CGazWidth.integer : 0;
...
/*
	if (tj_CGazInfo.integer)
	{
		char status[128];
		int i = 1;

		// consts
		Com_sprintf(status, sizeof(status), "speed: %d, gravity: %d", ps->speed, 800);
		CG_Text_Paint_Ext(5, i++ * 10, 0.2f, 0.2f, colorWhite, status, 0, 0, 0, &cgs.media.limboFont2);
		Com_sprintf(status, sizeof(status), "sprintSpeedScale: %f, runSpeedScale: %f", ps->sprintSpeedScale, ps->runSpeedScale);
		CG_Text_Paint_Ext(5, i++ * 10, 0.2f, 0.2f, colorWhite, status, 0, 0, 0, &cgs.media.limboFont2);

		// velocity
		Com_sprintf(status, sizeof(status), "Velocity: %.0f ups, %.0f ups, %.0f ups", vel[0], vel[1], vel[2]);
		CG_Text_Paint_Ext(5, i++ * 10, 0.2f, 0.2f, colorWhite, status, 0, 0, 0, &cgs.media.limboFont2);
		Com_sprintf(status, sizeof(status), "  xy module: %f ups", vel_size);
		CG_Text_Paint_Ext(5, i++ * 10, 0.2f, 0.2f, colorWhite, status, 0, 0, 0, &cgs.media.limboFont2);
		Com_sprintf(status, sizeof(status), "  xy angle: %f deg", vel_angle);
		CG_Text_Paint_Ext(5, i++ * 10, 0.2f, 0.2f, colorWhite, status, 0, 0, 0, &cgs.media.limboFont2);

		// view angles
		Com_sprintf(status, sizeof(status), "View angles: %f deg, %f deg", ps->viewangles[PITCH], ps->viewangles[YAW]);
		CG_Text_Paint_Ext(5, i++ * 10, 0.2f, 0.2f, colorWhite, status, 0, 0, 0, &cgs.media.limboFont2);
	}
*/
/*
	// setup's original CGaz HUD
	if (tj_drawCGaz.integer == 1)
	{
		...
		if (vel_relang > -90 && vel_relang < 90)
			CG_FillRect(SCREEN_CENTER_X + w * vel_relang / 180, y,
						1, h, colorOrange);
		...
		if (vel_relang > 0)
		{
			...
			if (ang > -90 && ang < 90)
				CG_FillRect(SCREEN_CENTER_X + w * ang / 180, y,
							1, h, colorGreen);
		}
		else
		{
...
static void CG_DrawOB(void)
{
	...
	if (!tj_drawOB.integer || tjg_disabled_ob.integer || tjg_recordMode.integer || physics.integer >= PHYSICS_DISABLED_OB || cg_thirdPerson.integer)
		return;

	psec = pmove_msec.integer / 1000.0;
	gravity = cg.ps.gravity;
	v0 = cg.ps.velocity[2];
	h0 = cg.ps.origin[2] + cg.ps.mins[2];
	...
	CG_Trace(&trace, start, vec3_origin, vec3_origin, end,
			 cg.ps.clientNum, CONTENTS_SOLID);
	...
	if (cg.ps.groundEntityNum != ENTITYNUM_NONE)
	{
		// jump ob
...
static void CG_DrawKeys(void)
{
	...
	if (tj_drawKeys.integer <= 0)
		return;

	// some checks
	if (tj_keysSize.value < 0 || tj_keysX.value < 0 || tj_keysY.value < 0
			|| tj_keysX.value > SCREEN_WIDTH || tj_keysY.value > SCREEN_HEIGHT)
		return;

	// Dini, add in here for any other keysets with skew effect or related.
	if (tj_drawKeys.value == 1)
		skew = tj_keysSize.value / 18;
	else
		skew = 0;
	...
	size = tj_keysSize.value / 3;
	i = (tj_drawKeys.integer - 1) % NUM_KEYS_SETS;
	// first (upper) row
	// sprint (upper left)
	x = tj_keysX.value + 2 * skew;
	y = tj_keysY.value;
...
static void CG_DrawSpeedBar(rectDef_t *rect)
{
	vec4_t bgcolour =	{	1.f,	1.f,	1.f,	0.3f	};

	static vec_t speed;
	int flags = BAR_LEFT | BAR_VERT | BAR_BG | BAR_BGSPACING_X0Y0;
	float frac;

	if (!tj_drawSpeedBar.integer)
		return;

	speed = VectorLength(cg.ps.velocity);

	// From 0 - 1. Max speed: e.g. 2500 ups / 2500 = 1  =>  2500 is max speed shown
	frac = speed / tj_speedBarMax.integer;

	CG_FilledBar(rect->x, rect->y + (rect->h * 0.1f), rect->w, rect->h * 0.84f, colorWhite, NULL, bgcolour, frac, flags);
...
static void CG_DrawAccelBar(rectDef_t *rect)
{
	vec4_t bgcolour =	{	1.f,	1.f,	1.f,	0.3f	};

	static vec_t speedpast, speednow;
	int flags = BAR_LEFT | BAR_VERT | BAR_BG | BAR_BGSPACING_X0Y0;
	float accel;
	int thistime, y, w, h;
	static int lasttime;

	if (!tj_drawAccelBar.integer)
		return;

	thistime = trap_Milliseconds();
	speednow = VectorLength(cg.ps.velocity);

	if (thistime > (lasttime + 250))
	{

		speedpast = VectorLength(cg.ps.velocity);

		lasttime = thistime;
	}

	accel = speednow - speedpast;

	/*
	s = va("Accel: %.4f", accel);
	sw = CG_Text_Width_Ext( s, 0.15f, 0, &cgs.media.limboFont1 );
	CG_Text_Paint_Ext( 10, 10, 0.2f, 0.2f, colorRed, s, 0, 0, 0, &cgs.media.limboFont1 );
	*/

	if (tj_drawAccelBar.integer == 1)
	{
		if (accel > 0)
			CG_FillRect(25, 25, 25, 15, colorF);
		else
			CG_FillRect(25, 25, 25, 15, colorWhite);

		if (accel >= 0)
			CG_FilledBar(rect->x, rect->y + (rect->h * 0.1f), rect->w, rect->h * 0.84f, colorF, NULL, bgcolour, accel / 200, flags);
		else if (accel < 0)
		{
			accel *= -1;
			CG_FilledBar(rect->x, rect->y + (rect->h * 0.1f), rect->w, rect->h * 0.84f, colorOrange, NULL, bgcolour, accel / 200, flags);
		}

		CG_DrawPic(rect->x, rect->y, rect->w, rect->h, cgs.media.hudSprintBar);
		CG_DrawRect_FixedBorder(25, 25, 25, 15, 1, colorWhite);
	}
	if (tj_drawAccelBar.integer == 2)
	{
		vec4_t	colorF50 = {0.2f, 0.6f, 0.8f, 0.5f};

		// Looks better, and no practical decrease in functionality.
		accel *= 0.5;
		if (accel > (SCREEN_WIDTH / 8))
			accel = (SCREEN_WIDTH / 8);
		if (accel < (SCREEN_WIDTH / 8) * -1)
			accel = (SCREEN_WIDTH / 8) * -1;
		...
		if (accel >= 0)
			CG_FillRect(SCREEN_CENTER_X + 1, y, w, h, colorF50);
		else if (accel < 0)
		{
			accel *= -1;
			w = accel;
			CG_FillRect(SCREEN_CENTER_X + 1 - w, y, w, h, colorF50);
		}
	}
...
static void CG_DrawTJCompass(void)
{
	int y = tj_compassY.integer;
	int h = tj_compassHeight.integer;
	float angle = cg.ps.viewangles[YAW];

	// This works, God knows why.. (god aka setup if he looks into it)
	float WTF = ((angle * 4) - (45 - (320 / 90)));

	if (!tj_drawCompass.integer)
		return;

	// The main 'blocks' - always drawn if enabled
	CG_FillRect(WTF,		y,	360, h, cg.compassColor1);
	CG_FillRect(WTF - 360,	y,	360, h, cg.compassColor3);
	CG_FillRect(WTF + 360,	y,	360, h, cg.compassColor2);
	CG_FillRect(WTF + 720,	y,	360, h, cg.compassColor4);
	// Avoids 'switching' @ 180
	CG_FillRect(WTF + 1080,	y,	360, h, cg.compassColor3);
	CG_FillRect(WTF - 720,	y,	360, h, cg.compassColor4);

	if (tj_drawCompass.integer == 2 || tj_drawCompass.integer == 3)
	{
		CG_FillRect(WTF + 180,	y,	1, h, cg.compassColor1_strong);
		CG_FillRect(WTF - 180,	y,	1, h, cg.compassColor3_strong);
		CG_FillRect(WTF + 540,	y,	1, h, cg.compassColor2_strong);
		CG_FillRect(WTF + 900,	y,	1, h, cg.compassColor4_strong);

		// Avoids 'switching' @ 180
		CG_FillRect(WTF + 1260,	y,	1, h, cg.compassColor3_strong);
		CG_FillRect(WTF - 540,	y,	1, h, cg.compassColor4_strong);

		if (tj_drawCompass.integer == 3)
		{
			// This draws the 45* angles.
			// Note: This is relative to 360* being 90*,
			// which means that 45 degrees is actually 11.25*
			// Which also means that 180 above is 45*.
			// 360 / 90 = 4  =>  180 / 4 = 45

			CG_FillRect(WTF + 45,		y,	1, h, cg.compassColor1_strong);
			CG_FillRect(WTF + 315,	y,	1, h, cg.compassColor1_strong);

			CG_FillRect(WTF - 315,	y,	1, h, cg.compassColor3_strong);
			CG_FillRect(WTF - 45,		y,	1, h, cg.compassColor3_strong);

			CG_FillRect(WTF + 675,	y,	1, h, cg.compassColor2_strong);
			CG_FillRect(WTF + 405,	y,	1, h, cg.compassColor2_strong);

			CG_FillRect(WTF + 1035,	y,	1, h, cg.compassColor4_strong);
			CG_FillRect(WTF + 765,	y,	1, h, cg.compassColor4_strong);

			// Avoids 'switching' @ 180
			CG_FillRect(WTF + 1395,	y,	1, h, cg.compassColor3_strong);
			CG_FillRect(WTF + 1125,	y,	1, h, cg.compassColor3_strong);

			CG_FillRect(WTF - 405,	y,	1, h, cg.compassColor4_strong);
			CG_FillRect(WTF - 675,	y,	1, h, cg.compassColor4_strong);
		}
	}
}


static void CG_DrawYawDirection(void)
{
	float angle = cg.ps.viewangles[YAW];
	static float pastangle;
	int thistime;
	static int lasttime;

	if (!tj_drawYaw.integer)
		return;

	thistime = trap_Milliseconds();
	if (thistime > (lasttime + 180))
	{
		pastangle = cg.ps.viewangles[YAW];
		lasttime = thistime;
	}

	trap_R_SetColor(cg.yawColor);

	/*Com_sprintf(status, sizeof(status), "Angle1: %f", angle);
	CG_Text_Paint_Ext(25, 50, 0.2f, 0.2f, colorWhite, status, 0, 0, 0, &cgs.media.limboFont2);
	Com_sprintf(status, sizeof(status), "Angle2: %f", pastangle);
	CG_Text_Paint_Ext(25, 60, 0.2f, 0.2f, colorWhite, status, 0, 0, 0, &cgs.media.limboFont2);*/

	if (angle > pastangle)
		//Com_sprintf(status, sizeof(status), "Mouse movement direction: Left");
		CG_DrawPic(298, 239, 12, 12, cgs.media.ArrowL);
	if (angle < pastangle)
		//Com_sprintf(status, sizeof(status), "Mouse movement direction: Right");
		CG_DrawPic(330, 239, 12, 12, cgs.media.ArrowR);
	//if (angle == pastangle)
	//Com_sprintf(status, sizeof(status), "Mouse movement direction: Not moving");

	//CG_Text_Paint_Ext(25, 25, 0.2f, 0.2f, colorWhite, status, 0, 0, 0, &cgs.media.limboFont2);
...
static void CG_DrawTimer(void)
{
	...
	float sizex, sizey;
	int x, y, w;
	int timerunNum, order, num, i;
	static int nowTime = 0, bestTime = 0;
	vec4_t	color;
	static int time4average, numTimesChecked = 0, checkpointTimes[MAX_TIMERUN_CHECKPOINTS], checkpointDiffs[MAX_TIMERUN_CHECKPOINTS];
	static int cpIsFaster[MAX_TIMERUN_CHECKPOINTS];
	static int isFaster;
...
/*
	///////////////////////////////////////////////////////////////////////////
	// CBA to add any of this somewhere else atm ;D
	///////////////////////////////////////////////////////////////////////////
	...
	if (tj_autodemo.integer && !cg.demoPlayback)
	{
		...
		if (cg.runsave)
		{
			if (!rs_keep)
			{
				if (tj_autodemo.integer == 1)
					CG_AddPMItem(PM_MESSAGE, "^dTJMod^f:^g Stopping and saving demo..\n", cgs.media.voiceChatShader);
			...
			if (cg.time > rs_time + 2000)
			{
				...
				if (tj_demoname.integer == 1)
					name = va("demos/%s_%s_%s.dm_84", cgs.clientinfo[cg.clientNum].cleanname, cgs.rawmapname, cg.runsavename);
				else if (tj_demoname.integer == 2)
					name = va("demos/%s_%s_%s.dm_84", cgs.rawmapname, cgs.clientinfo[cg.clientNum].cleanname, cg.runsavename);
				else
				...
				if (tj_autodemo.integer == 1)
					CG_AddPMItem(PM_MESSAGE, "^dTJMod^f:^g Demo saved!\n", cgs.media.voiceChatShader);
...
*/
	...
	if ((!millistart && startTime > 0) || cg.timerunSpec)
	{
	...
	sizex = sizey = 0.1f;
	sizex *= tj_timerSizeX.value;
	sizey *= tj_timerSizeY.value;
	...
	if (cg.resettimeruns)
	{
		for (i = 0; i < MAX_TIMERUN_CHECKPOINTS; i++)
		{
			checkpointTimes[i] = 0;
			checkpointDiffs[i] = 0;
			cpIsFaster[i] = 0;
			cg.timerunBestTime[i] = 0;
			cg.timerunBestTime[i * 2] = 0;
			cg.timerunLastTime[i] = 0;
			cg.timerunLastTime[i * 2] = 0;
		}
		needsReset = 1;
	}

	if (cg.timerunActive)
		if (!cg.timerunBestTime[timerunNum] || cg.timerunLastTime[timerunNum] < cg.timerunBestTime[timerunNum])
			bestTime = cg.timerunBestTime[timerunNum] = cg.timerunLastTime[timerunNum];
	...
	{
		cg.finishedTime = cg.finishedTimeDiff = isFaster = 0;
		millistart = 0;
		numTimesChecked = 0;
		needsReset = 0;
		cg.resettimeruns = 0;
	...
	if (cg.finishedTime)
	{
		...
		dmin = dsec = dmilli = isFaster = 0;


	if (!tj_drawTimer.integer)
		return;

	if (tj_timerColorChange.integer && nowTime > bestTime && bestTime > 0)
		Vector4Set(color, cg.timerColorSlower[0], cg.timerColorSlower[1], cg.timerColorSlower[2], cg.timerColorSlower[3]);
	else
		Vector4Set(color, cg.timerColor[0], cg.timerColor[1], cg.timerColor[2], cg.timerColor[3]);

	if (tj_drawTimer.integer >= 9)
	{
		Com_sprintf(status, sizeof(status), va("min %i", min));
		CG_Text_Paint_Ext(500, 10, sizex, sizey, color, status, 0, 0, 0, &cgs.media.limboFont2);
		Com_sprintf(status, sizeof(status), va("sec %i", sec));
		CG_Text_Paint_Ext(500, 20, sizex, sizey, color, status, 0, 0, 0, &cgs.media.limboFont2);
		Com_sprintf(status, sizeof(status), va("ms %i", milli));
		CG_Text_Paint_Ext(500, 30, sizex, sizey, color, status, 0, 0, 0, &cgs.media.limboFont2);
		Com_sprintf(status, sizeof(status), va("now %i", nowTime));
		CG_Text_Paint_Ext(500, 40, sizex, sizey, color, status, 0, 0, 0, &cgs.media.limboFont2);
		Com_sprintf(status, sizeof(status), va("cg %i", cg.time));
		CG_Text_Paint_Ext(500, 50, sizex, sizey, color, status, 0, 0, 0, &cgs.media.limboFont2);
		Com_sprintf(status, sizeof(status), va("cg-now %i", cg.time - nowTime));
		CG_Text_Paint_Ext(500, 60, sizex, sizey, color, status, 0, 0, 0, &cgs.media.limboFont2);
	}

	if (cg.finishedTime && min + sec + milli != dmin + dsec + dmilli)
		Com_sprintf(status, sizeof(status), va("%02d:%02d.%03d (%s%02d:%02d.%03d)", min, sec, milli, c, dmin, dsec, dmilli));
	else
		Com_sprintf(status, sizeof(status), va("%02d:%02d.%03d", min, sec, milli));

	if (isFaster)
		Vector4Set(color, cg.chColorFast[0], cg.chColorFast[1], cg.chColorFast[2], cg.chColorFast[3]);
	...
	// Draw checkpoints
	if (cg.timerunChecked)
	{
		checkpointTimes[numTimesChecked + 1] = cg.timerunCheckpointTime;
		checkpointDiffs[numTimesChecked + 1] = cg.timerunCheckpointDiff;
		cpIsFaster[numTimesChecked + 1] = cg.timerunCheckIsFaster;
		numTimesChecked++;
		cg.timerunChecked = 0;
	}

	if (tj_drawTimerChecks.integer)
	{

		if (!tj_timerChecksMax.integer)
			num = 0;
		else
			num = numTimesChecked - tj_timerChecksMax.integer;

		if (num < 0)
			num = sqrt(num * num);

		if (numTimesChecked < tj_timerChecksMax.integer + 1)
			num = 0;

		for (i = numTimesChecked; i > num ; i--)
		{
			cmil = checkpointTimes[i];
			cmin = cmil / 60000;
			cmil -= cmin * 60000;
			csec = cmil / 1000;
			cmil -= csec * 1000;

			cdmil = checkpointDiffs[i];
			cdmin = cdmil / 60000;
			cdmil -= cdmin * 60000;
			cdsec = cdmil / 1000;
			cdmil -= cdsec * 1000;
			...
			sizex = sizey = 0.1f;
			sizex *= tj_timerChecksSizeX.value;
			sizey *= tj_timerChecksSizeY.value;
			x = tj_timerChecksX.value;
			y = tj_timerChecksY.value;

			if (!tj_timerChecksExt.integer)
				ext1 = ext2 = "";
			else
			{
				if (tj_timerChecksExt.integer >= 1)
					ext1 = va("[%i] ", i);
				if (tj_timerChecksExt.integer == 2)
					ext2 = va(" ^z(%02d:%02d.%03d)", cmin, csec, cmil);
				else
					ext2 = "";
			}

			if (!tj_timerChecksOrder.integer)
				order = y + tj_timerChecksOffset.integer * (numTimesChecked - i);
			else
				order = y + tj_timerChecksOffset.integer * i;

			if (!bestTime)
			{
				Com_sprintf(status, sizeof(status), va("%s%02d:%02d.%03d", ext1, cmin, csec, cmil));
				w = CG_Text_Width_Ext(status, sizex, 0, &cgs.media.limboFont2) / 2;
				CG_Text_Paint_Ext(x - w, order, sizex, sizey, cg.chColorNone, status, 0, 0, 0, &cgs.media.limboFont2);
			}
			else if (cdmin + cdsec + cdmil == 0)
			{
				Com_sprintf(status, sizeof(status), va("%s%02d:%02d.%03d%s", ext1, cdmin, cdsec, cdmil, ext2));
				w = CG_Text_Width_Ext(status, sizex, 0, &cgs.media.limboFont2) / 2;
				CG_Text_Paint_Ext(x - w, order, sizex, sizey, cg.chColorSame, status, 0, 0, 0, &cgs.media.limboFont2);
			}
			else if (cpIsFaster[i])
			{
				Com_sprintf(status, sizeof(status), va("%s-%02d:%02d.%03d%s", ext1, cdmin, cdsec, cdmil, ext2));
				w = CG_Text_Width_Ext(status, sizex, 0, &cgs.media.limboFont2) / 2;
				CG_Text_Paint_Ext(x - w, order, sizex, sizey, cg.chColorFast, status, 0, 0, 0, &cgs.media.limboFont2);
			}
			else
			{
				Com_sprintf(status, sizeof(status), va("%s+%02d:%02d.%03d%s", ext1, cdmin, cdsec, cdmil, ext2));
				w = CG_Text_Width_Ext(status, sizex, 0, &cgs.media.limboFont2) / 2;
				CG_Text_Paint_Ext(x - w, order, sizex, sizey, cg.chColorSlow, status, 0, 0, 0, &cgs.media.limboFont2);
			}
		}
	}

	// Div stuff, doesn't really belong here but care
	if (tj_drawTimer.integer != 10)
		return;

	Com_sprintf(status, sizeof(status), va("Seconds passed: %i", time4average));
	CG_Text_Paint_Ext(10, 10, sizex * 0.8, sizey * 0.8, cg.timerColor, status, 0, 0, 0, &cgs.media.limboFont2);

	Com_sprintf(status, sizeof(status), va("Jump count: %i", cg.ps.curWeapHeat));
	CG_Text_Paint_Ext(10, 25, sizex * 0.8, sizey * 0.8, cg.timerColor, status, 0, 0, 0, &cgs.media.limboFont2);

	Com_sprintf(status, sizeof(status), va("Run Distance: %.0f", cg.runDistance));
	CG_Text_Paint_Ext(10, 40, sizex * 0.8, sizey * 0.8, cg.timerColor, status, 0, 0, 0, &cgs.media.limboFont2);

	time4average = sec + (min * 60) + (milli / 1000);
	Com_sprintf(status, sizeof(status), va("Average speed: %.0f", cg.runDistance / time4average));
	CG_Text_Paint_Ext(10, 55, sizex * 0.8, sizey * 0.8, cg.timerColor, status, 0, 0, 0, &cgs.media.limboFont2);

	Com_sprintf(status, sizeof(status), va("Topspeed: %.0f", cg.topSpeed));
	CG_Text_Paint_Ext(10, 70, sizex * 0.8, sizey * 0.8, cg.timerColor, status, 0, 0, 0, &cgs.media.limboFont2);

	/*
	Com_sprintf(status, sizeof(status), va("Current timerun: %i (now: %i)", timerunNum, nowTime));
	CG_Text_Paint_Ext(x-200, y+50, sizex, sizey, cg.timerColor, status, 0, 0, 0, &cgs.media.limboFont2);

	Com_sprintf(status, sizeof(status), va("Best time: %i", bestTime));
	CG_Text_Paint_Ext(x-200, y-100, sizex, sizey, cg.timerColor, status, 0, 0, 0, &cgs.media.limboFont2);
	*/
}


static void CG_DrawJumps(void)
{
	char status[128];
	static vec_t speed, jumpSpeeds[128];
	static int lastJumpCount = 0, thisJumpCount, needsReset = 0, numTimes = 0;
	int y, i;

	if (!tj_drawJumps.integer)
		return;

	speed = sqrt(cg.ps.velocity[0] * cg.ps.velocity[0] + cg.ps.velocity[1] * cg.ps.velocity[1]);
	thisJumpCount = cg.ps.curWeapHeat;

	if (thisJumpCount > lastJumpCount)
	{
		jumpSpeeds[thisJumpCount] = speed;
		numTimes++;
		lastJumpCount = thisJumpCount;
	}

	y = 10;
	for (i = 0; i < numTimes; i++)
	{
		Com_sprintf(status, sizeof(status), va("%.0f", jumpSpeeds[i + 1]));
		CG_Text_Paint_Ext(5, y + 10 * i, 0.2f, 0.2f, cg.timerColor, status, 0, 0, 0, &cgs.media.limboFont2);
	}

	// Clearing out stuff, but keep it until timerun has started again :)
	if (!cg.timerunActive)
		needsReset = 1;
	if (needsReset && cg.timerunActive)
		numTimes = lastJumpCount = needsReset = 0;
}

static void CG_DrawStartSpeed(void)
{
	char status[128];
	static vec_t speed;
	static int needsReset = 0;

	if (!tj_drawStartSpeed.integer)
		return;
	...
	CG_Text_Paint_Ext(tj_startSpeedX.integer, tj_startSpeedY.integer, 0.3f, 0.3f, colorWhite, status, 0, 0, 0, &cgs.media.limboFont2);
}
...
static void CG_DrawTJSpeed(void)
{
	...
	sizex = sizey = 0.1f;
	sizex *= tj_speedSizeX.integer;
	sizey *= tj_speedSizeY.integer;

	x = tj_speedX.integer;
	y = tj_speedY.integer;


	switch (tj_drawSpeed2.integer)
	{
		case 1:
			Com_sprintf(status, sizeof(status), va("%.0f", speed));
			break;
	...
	w = CG_Text_Width_Ext(status, sizex, 0, &cgs.media.limboFont2) / 2;
	if (tj_drawSpeed2.integer == 8)
		w = 0;

	CG_Text_Paint_Ext(x - w, y, sizex, sizey, cg.speedColor, status, 0, 0, 0, &cgs.media.limboFont2);
}


static void CG_DrawAvgSpeed(void)
{
	char status[128];
	static int millistart;
	float sizex, sizey;
	int x, y, seconds;
	static float avgSpeed;

	if (!tj_drawavgspeed.integer)
		return;

	if (!millistart)
		millistart = cg.time;

	seconds = cg.time - millistart;
	seconds /= 1000;

	sizex = sizey = 0.1f;
	sizex *= tj_avgspeedSizeX.integer;
	sizey *= tj_avgspeedSizeY.integer;

	x = tj_avgspeedX.integer;
	y = tj_avgspeedY.integer;

	avgSpeed = cg.runDistance / seconds;
	...
	switch (tj_drawavgspeed.integer)
	{
		case 1:
			Com_sprintf(status, sizeof(status), va("%.0f", avgSpeed));
			break;
		case 2:
			Com_sprintf(status, sizeof(status), va("Average speed: %.0f", avgSpeed));
			break;
	}

	CG_Text_Paint_Ext(x, y, sizex, sizey, cg.avgspeedColor, status, 0, 0, 0, &cgs.media.limboFont2);
}


static void CG_DrawRunDist(void)
{
	char status[128];
	float sizex, sizey;
	int x, y;

	if (!tj_drawrundist.integer)
		return;

	sizex = sizey = 0.1f;
	sizex *= tj_rundistSizeX.integer;
	sizey *= tj_rundistSizeY.integer;

	x = tj_rundistX.integer;
	y = tj_rundistY.integer;

	if (tj_drawrundist.integer == 1)
		Com_sprintf(status, sizeof(status), va("%.0f", cg.runDistance));
	else if (tj_drawrundist.integer == 2)
		Com_sprintf(status, sizeof(status), va("Run distance: %.0f", cg.runDistance));

	CG_Text_Paint_Ext(x, y, sizex, sizey, cg.rundistColor, status, 0, 0, 0, &cgs.media.limboFont2);
}


static void CG_DrawTJSpeedGFX(void)
{
	int i;
	int x, y, w, h;
	int tx, ty, tw;
	int vx, vh, vy, vw;
	static int nextTime, numTimes = 0;
	static vec_t pre[SCREEN_WIDTH], temp[SCREEN_WIDTH];
	static vec_t speed, topSpeed;
	vec4_t fade2;
	...
	/////////////////////////////////////////////////
	// Graphical Speed Settings explanations \ notes
	// ---------------------------------------------
	// tj_drawGS;		1: horizontal double ||NEED2DO|| 2: hor single, 3: verical double, 4: vertical single, 5:box.
	// tj_gsX;			X position, autocenters with - tj_gsWidth / 2.
	// tj_gsY;			Y offset to the default which is (screen_height - height of the speed bars).
	// tj_gsHeight;		The max height, will be max drawing height for tj_drawGS 1-4 and same for box height.
	// tj_gsWidth;		The main width, always applicable.
	// tj_gsInterval;	The interval (how often) it will update the speed. 0 by default means constant.
	//					This is "how far into the past" you'll see your speed.
	// tj_gsColor1;		Main, Color 1, applies to tj_drawGS 1 mid bar (current speed), and similar.
	// tj_gsColor2;		Secondary, Color 2, applies to past speed drawing at sides of tj_drawGS 1, and similar.
	// tj_gsColor3;		Extra, Color 3, applies to e.g. tj_gsCeiling if enabled.
	// tj_gsAlpha1;		These toggles opacity for corresponding color.
	// tj_gsAlpha2;		-||-
	// tj_gsAlpha3;		-||-
	// tj_gsCeiling;	Draws a line illustrating top speed e.g. above the mid bar at tj_drawGS 1.
	// tj_gsMax;		The max speed the speedbar will show. 20 by default.
	//					Note: 20 meaning (speed / 20), which makes a speed of 2000 UPS = 100 in height.
	//					Higher values means less variation and possiblity to show higher speeds in a smaller height.
	//					Lower values means more variation and that it will take more space.
	//					e.g. a value of 5 would make 1000 ups 200 high, twice as high as 2000 ups with a value of 20.
	//					TIP:	Find a "max" relevant speed you'd like to show, then a nice max height (tj_gsHeight)
	//							and do (max_relevant_speed / tj_gsHeight) = value for tj_gsMax. (default: 2000/100 = 20)
	// tj_gsSpacing;	Will add a spacing between main bar and side bars.
	// tj_gsSpacing2;	Will add a spacing between each bar \ line drawn.
	// tj_gsW;			Secondary width, 1 by default, applies to e.g. side lines drawn on the sides of tj_drawGS 1.
	// tj_gsFade;		Will make the e.g. the opacity of the outer lines of tj_drawGS 1 lower, hence a fadeout effect.
	// tj_gsImage		Draw an image behind center bar that singifies the speed.
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	if (!tj_drawGS.integer)
		return;

	speed = sqrt(cg.ps.velocity[0] * cg.ps.velocity[0] + cg.ps.velocity[1] * cg.ps.velocity[1]);
	Vector4Set(fade2, cg.gsColor2[0], cg.gsColor2[1], cg.gsColor2[2], cg.gsColor2[3]);

	// Middle bar for tj_drawGS 1
	w = tj_gsWidth.integer;
	x = tj_gsX.integer - w / 2;
	h = speed / tj_gsMax.integer;
	if (h > tj_gsHeight.integer)
		h = tj_gsHeight.integer;

	y = 480 - h + tj_gsY.integer;

	if (tj_gsImage.integer)
		CG_DrawPic(x - 6, 480 - 96, 32, 96, cgs.media.gsT1);

	CG_FillRect(x, y, w, h, cg.gsColor1);


	/////////////////////////////////////////////////////////////////////////////////////////////
	// Start tj_drawGS 1
	// Actual stuff that makes it move
	if (cg.time > nextTime)
	{
		nextTime = cg.time + tj_gsInterval.integer;

		if (numTimes > SCREEN_WIDTH)
		{
			for (i = 0; i < SCREEN_WIDTH - 1; i++)
			{
				temp[i] = temp[i + 1];
			}
			numTimes = SCREEN_WIDTH - 1;
		}

		temp[numTimes] = speed / tj_gsMax.integer;

		for (i = 0; i < SCREEN_WIDTH - 1; i++)
		{
			pre[i + 1] = temp[numTimes - i];
		}

		pre[0] = speed / tj_gsMax.integer;

		numTimes++;
	}

	// Draws the moving stuff
	for (i = 0; i < SCREEN_WIDTH; i++)
	{
		if (tj_gsFade.integer == 1 && i > 180)
			fade2[3] -= (i / 120) - 2;
		if (tj_gsFade.integer == 2 && i > 180)
			fade2[3] -= (i / 100) - 1;

		vw = tj_gsW.integer;
		vx = x - vw * (i + 1) - tj_gsSpacing2.integer * i;
		vh = pre[i];
		if (vh > tj_gsHeight.integer)
			vh = tj_gsHeight.integer;
		vy = 480 - vh + tj_gsY.integer;

		// Draw to the  left
		CG_FillRect(vx - tj_gsSpacing.integer, vy, vw, vh, fade2);

		// Draw to the right too
		vx = x + vw * (i) + w;
		CG_FillRect(vx + tj_gsSpacing2.integer * i + tj_gsSpacing.integer, vy, vw, vh, fade2);
	}
	// End tj_drawGS 1
	/////////////////////////////////////////////////////////////////////////////////////////////

	// Draw a "floor" line for the topspeed, jff
	if (tj_gsCeiling.integer)
	{
		if (speed > topSpeed)
			topSpeed = speed;

		tw = topSpeed / tj_gsWidth.integer;
		tx = tj_gsX.integer - tw / 2;
		ty = 480 - tw + tj_gsY.integer;

		if (tj_gsCeiling.integer == 1)
			CG_FillRect(tx, ty - 2, tw, 2, cg.gsColor3);
		else if (tj_gsCeiling.integer == 2)
			CG_FillRect(tj_gsX.integer - w / 2, ty - 2, w, 2, cg.gsColor3);
	}
}
...
static qboolean CG_DrawFollow(void)
{
	...
	{
		CG_DrawStringExt(INFOTEXT_STARTX, 118, CG_TranslateString(va("Following %s", cgs.clientinfo[cg.snap->ps.clientNum].cleanname)), colorWhite, qtrue, qtrue, BIGCHAR_WIDTH / 2, BIGCHAR_HEIGHT, 0);
	}
...
void CG_DrawCompassIcon(float x, float y, float w, float h, vec3_t origin, vec3_t dest, qhandle_t shader)
{
	...
	//	else
	angles[YAW] = AngleSubtract(cg.ps.viewangles[YAW], angles[YAW]);
...
static void CG_DrawNewCompass(void)
{
	...
	angle = (cg.ps.viewangles[YAW] + 180.f) / 360.f - (0.125f);
	...
	for (i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if (cg.ps.clientNum == i || !cgs.clientinfo[i].infoValid || cg.ps.persistant[PERS_TEAM] != cgs.clientinfo[i].team)
			continue;
		...
		CG_DrawCompassIcon(basex, basey, basew, baseh, cg.ps.origin, cent->lerpOrigin, cent->voiceChatSprite);
	}
	//	}

	/*if( !(cgs.ccFilter & CC_FILTER_DESTRUCTIONS) ) {
		// draw explosives if an engineer
		if ( cg.ps.stats[ STAT_PLAYER_CLASS ] == PC_ENGINEER ) {
			for ( i = 0; i < snap->numEntities; i++ ) {
				...
				if ( cent->currentState.teamNum == 1 && cg.ps.persistant[PERS_TEAM] == TEAM_AXIS )
					continue;
				else if ( cent->currentState.teamNum == 2 && cg.ps.persistant[PERS_TEAM] == TEAM_ALLIES )
					continue;

				CG_DrawCompassIcon( basex, basey, basew, baseh, cg.ps.origin, cent->lerpOrigin, cgs.media.compassDestroyShader );
			}
		}
	}*/
	...
	if (cg.ps.stats[ STAT_PLAYER_CLASS ] == PC_MEDIC)
	{
		for (i = 0; i < snap->numEntities; i++)
		{
			...
			{
				if (!cgs.clientinfo[ent->clientNum].infoValid || cg.ps.persistant[PERS_TEAM] != cgs.clientinfo[ent->clientNum].team)
					continue;

				CG_DrawCompassIcon(basex, basey, basew, baseh, cg.ps.origin, ent->pos.trBase, cgs.media.medicReviveShader);
			}
		}
	}
	//	}
	...
	/*	if( !(cgs.ccFilter & CC_FILTER_DESTRUCTIONS) ) {
			...
			if ( cg.ps.stats[ STAT_PLAYER_CLASS ] == PC_ENGINEER ) {
				for ( i = 0; i < snap->numEntities; i++ ) {
					...
					if ( cent->currentState.teamNum != cg.ps.persistant[PERS_TEAM] && cent->currentState.teamNum != 3 )
						continue;

					CG_DrawCompassIcon( basex, basey, basew, baseh, cg.ps.origin, cent->lerpOrigin, cgs.media.compassConstructShader );
				}
			}
		}*/
	...
	/*	if( !(cgs.ccFilter & CC_FILTER_WAYPOINTS) ) {
			...
			for ( i = 0; i < snap->numEntities; i++ ) {
				...
				case WAYP_ATTACK: CG_DrawCompassIcon( basex, basey, basew, baseh, cg.ps.origin, cent->currentState.pos.trBase, cgs.media.waypointCompassAttackShader ); break;
				case WAYP_DEFEND: CG_DrawCompassIcon( basex, basey, basew, baseh, cg.ps.origin, cent->currentState.pos.trBase, cgs.media.waypointCompassDefendShader ); break;
				case WAYP_REGROUP: CG_DrawCompassIcon( basex, basey, basew, baseh, cg.ps.origin, cent->currentState.pos.trBase, cgs.media.waypointCompassRegroupShader ); break;
				}
			}
		}*/
	...
	for (i = 0; i < snap->numEntities; i++)
	{
		...
		CG_DrawCompassIcon(basex, basey, basew, baseh, cg.ps.origin, ent->pos.trBase, cgs.media.buddyShader);
	}
	//	}
}
...
static int CG_PlayerAmmoValue(int *ammo, int *clips, int *akimboammo)
{
	...
	*ammo = *clips = *akimboammo = -1;
	...
	// Dini, Note, hides the ammo count, do this for all @ unlimited
	switch (weap)  		// some weapons don't draw ammo count text
	{
		...
		case WP_SMOKE_BOMB:
		case WP_BINOCULARS:
			// Dini, dont show these anymore, because it's unlimited
...
static void CG_DrawWeapRecharge(rectDef_t *rect)
{
	...
	flags = BAR_LEFT | BAR_VERT | BAR_BG;
...
static void CG_DrawPlayerStats(void)
{
	...
	else if (tj_drawHealth.integer)
	{
		str = va("%i", cg.snap->ps.stats[STAT_HEALTH]);
		w = CG_Text_Width_Ext(str, 0.25f, 0, &cgs.media.limboFont1);
		CG_Text_Paint_Ext(SKILLS_X - 50 - w, 480 - 4, 0.15f, 0.15f, colorWhite, str, 0, 0, ITEM_TEXTSTYLE_SHADOWED, &cgs.media.limboFont1);
	}
...
void CG_DrawActive(stereoFrame_t stereoView)
{
	...
	/*
	vmCvar_t	tj_drawminiview;
	vmCvar_t	tj_miniviewx;
	vmCvar_t	tj_miniviewy;
	vmCvar_t	tj_minivieww;
	vmCvar_t	tj_miniviewh;
	vmCvar_t	tj_miniviewdist;
	vmCvar_t	tj_miniviewangle;
	vmCvar_t	tj_miniviewangle2;
	...
	*/
	...
	// Fixme, default values, sizing, angles and dist and origin and DRAWING ROCKETS/MODELS:?
	// Miniview
	if (tj_drawminiview.integer)
	{
		int x, y, w, h, dist, angle, angle2;

		x = tj_miniviewx.integer;
		y = tj_miniviewy.integer;
		w = tj_minivieww.integer;
		h = tj_miniviewh.integer;
		dist = tj_miniviewdist.integer;
		angle = tj_miniviewangle.integer;
		angle2 = tj_miniviewangle2.integer;
