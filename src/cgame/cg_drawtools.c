...
flags:
	BAR_LEFT
	BAR_CENTER			// direction is 'right' by default and orientation is 'horizontal'
	BAR_VERT
	BAR_BG				// background contrast box (bg set with bgColor of 'NULL' means use default bg color (1,1,1,0.25)
	BAR_BGSPACING_X0Y5	// some bars use different sorts of spacing when drawing both an inner and outer box
	BAR_LERP_COLOR		// use an average of the start and end colors to set the fill color
==============
*/
...
void CG_HorizontalPercentBar(float x, float y, float width, float height, float percent)
{
	...
	CG_FilledBar(x, y, width, height, color, NULL, bgcolor, percent, BAR_BG);
}
