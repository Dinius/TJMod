...
static void CG_ItemPickup(int itemNum)
{
	...
	if (bg_itemlist[itemNum].giType == IT_WEAPON)
	{
		...
		if (cg_autoswitch.integer && cg.ps.weaponstate != WEAPON_RELOADING)
		{
...
==============
*/
// Dini, Note, INITIATES weapon sound, blood splatter, etcetc
...
void CG_EntityEvent(centity_t *cent, vec3_t position)
{
	...
	switch (event)
	{
		...
		case EV_WEAP_OVERHEAT:
			...
			if (es->number == cg.snap->ps.clientNum)
			{
				cg.ps.weapAnim = ((cg.ps.weapAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) | PM_IdleAnimForWeapon(cg.snap->ps.weapon);
		...
		case EV_RAILTRAIL:
			...
			//
		case EV_MISSILE_HIT:
			if (tj_hide.integer || !tj_effects.integer)
				break;
