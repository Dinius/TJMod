...
void CG_DemoClick(int key, qboolean down)
{
	...
	switch (key)
	{
		...
		case K_F5:
			...
			return;
		default:
			// FIXME setup - quick hack to allow bindings in the demo
			// replaying mode, maybe it should be extended for other modes
			// with special keys handling too
			if (down)
			{
...
/*
 * Draws timerun stats overlay.
 */
void CG_TimerunStatsDraw()
{
	/*
	 * TODO
	 * timerun name: CG_ConfigString(CS_TIMERUNS + timerunNum)
	 * timerun time: timerunStats.times[timerunNum]
	 */

	// temporary debug code
	float diff = cgs.timerunStats.fadeTime - cg.time;
	timerunStats_t	*ts = &cgs.timerunStats;
	int				i;
	const char		*name;
	static int		lastPrinted;

	if (cgs.timerunStats.show == SHOW_OFF)
		return;

	// fade-in effects
	if (diff <= 0.0f && cgs.timerunStats.show == SHOW_SHUTDOWN)
	{
		cgs.timerunStats.show = SHOW_OFF;
		return;
	}

	if (lastPrinted + 2000 < cg.time)
	{
		CG_Printf("CG_TimerunStatsDraw: clientID: %d\n", ts->clientID);
		for (i = 0; i < MAX_TIMERUNS && *(name = CG_ConfigString(CS_TIMERUNS + i)); i++)
			CG_Printf("%s: %d\n", name, ts->times[i]);
		lastPrinted = cg.time;
	}
}
