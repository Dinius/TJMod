...
void CG_DrawPlayerWeaponIcon(rectDef_t *rect, qboolean drawHighlighted, int align, vec4_t *refcolor)
{
	...
	if (cg.ps.grenadeTimeLeft)  	// grenades and dynamite set this
	{

		// these time differently
		if (realweap == WP_DYNAMITE)
		{
			if (((cg.grenLastTime) % 1000) > ((cg.ps.grenadeTimeLeft) % 1000))
			{
		...
		else
		{
			if (((cg.grenLastTime) % 1000) < ((cg.ps.grenadeTimeLeft) % 1000))
			{
				switch (cg.ps.grenadeTimeLeft / 1000)
				{
					case 3:
						trap_S_StartLocalSound(cgs.media.grenadePulseSound4, CHAN_LOCAL_SOUND);
						break;
		...
		scale = (float)((cg.ps.grenadeTimeLeft) % 1000) / 100.0f;
		halfScale = scale * 0.5f;

		cg.grenLastTime = cg.ps.grenadeTimeLeft;
	}
	else
	{
		scale = halfScale = 0;
...
#if 0
			if (!(cg.ps.weapon) ||
					WEAPS_ONE_HANDED & (1 << (cg.ps.weapon))
			   )
			{
...
void CG_MouseEvent(int x, int y)
{
	switch (cgs.eventHandling)
	{
		...
		default:
			...
			// default handling
			if ((cg.ps.pm_type == PM_NORMAL ||
					cg.ps.pm_type == PM_SPECTATOR) &&
...
void CG_KeyEvent(int key, qboolean down)
{
	switch (cgs.eventHandling)
	{
		...
		default:
			...
			if ((cg.ps.pm_type == PM_NORMAL ||
					(cg.ps.pm_type == PM_SPECTATOR && cg.showScores == qfalse)))
			{
