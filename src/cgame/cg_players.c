...
void CG_NewClientInfo(int clientNum)
{
	...
	v = Info_ValueForKey(configstring, "tjk");
	newInfo.tjkey = atoi(v);

	v = Info_ValueForKey(configstring, "tbt");
	newInfo.timerunBT = atoi(v);
...
static qboolean CG_PlayerShadow(centity_t *cent, float *shadowPlane, qboolean hidden)
{
	vec3_t			end;
	trace_t			trace;
	float			dist, distFade;
	...
	*shadowPlane = trace.endpos[2] + 1;
	...
	if (!(cent->currentState.eFlags & EF_ZOOMING) && (dist > SHADOW_MIN_DIST))
	{
		...
		if (!hidden)
			CG_ImpactMark(cgs.media.shadowTorsoShader, origin, projection, 18.0f, cent->lerpAngles[ YAW ], distFade, distFade, distFade, distFade, -1);
	...
	if (dist < SHADOW_MAX_DIST)  	// show more detail
	{
		...
		for (tagIndex = 0; shadowParts[tagIndex].tagname; tagIndex++)
		{
			...
			for (subIndex = 0; (subIndex = CG_GetOriginForTag(cent, &cent->pe.bodyRefEnt, shadowParts[tagIndex].tagname, subIndex, origin, axis)) >= 0; subIndex++)
			{
				...
				// DINI, FIXME, TAKING THIS AWAY = EVERYTHING GOES TO HELL : WHYES
				CG_ImpactMark(shadowParts[ tagIndex ].shader, origin, projection, shadowParts[ tagIndex ].size, angles[ YAW ], distFade, distFade, distFade, distFade, -1);
			}
		}
	}
...
void CG_Player(centity_t *cent)
{
	...
	if (tj_hide.integer && ci->clientNum != cg.clientNum && Distance(cgsnap->lerpOrigin, cent->lerpOrigin) < tj_hideDist.integer)
		return;

	// Hide any hidden players, naturally :o
	if (ci->hideme || tj_hide.integer == 2)
		hidden = qtrue;
	...
	// add the
	shadow = CG_PlayerShadow(cent, &shadowPlane, hidden);
