...
void CG_Respawn(qboolean revived)
{
	...
	cg.ps.weapAnim = ((cg.ps.weapAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) | PM_IdleAnimForWeapon(cg.snap->ps.weapon);	// reset weapon animations
	cg.ps.weaponstate = WEAPON_READY;	// hmm, set this?  what to?
	...
	if (cg.ps.stats[STAT_PLAYER_CLASS] == PC_COVERTOPS)
	{
