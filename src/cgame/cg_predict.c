...
// cg_predict.c -- this file generates cg.ps by either
...
Generates cg.ps by interpolating between
...
static void CG_TouchItem(centity_t *cent)
{
	...
	if (!BG_PlayerTouchesItem(&cg.ps, &cent->currentState, cg.time))
	{
		return;
	}
	...
	if (!BG_CanItemBeGrabbed(&cent->currentState, &cg.ps, cgs.clientinfo[cg.snap->ps.clientNum].skill, cgs.clientinfo[cg.snap->ps.clientNum].team))
	{
	...
	if (item->giType == IT_WEAPON)
	{
		...
		{
			if (!COM_BitCheck(cg.ps.weapons, item->giTag))
			{
	...
	if (cg.ps.persistant[PERS_TEAM] == TEAM_AXIS &&
			item->giTag == PW_REDFLAG)
		return;
	if (cg.ps.persistant[PERS_TEAM] == TEAM_ALLIES &&
			item->giTag == PW_BLUEFLAG)
		return;


	// grab it
	BG_AddPredictableEventToPlayerstate(EV_ITEM_PICKUP, cent->currentState.modelindex , &cg.ps);
	...
	if (item->giType == IT_WEAPON)
	{
		COM_BitSet(cg.ps.weapons, item->giTag);

		if (!cg.ps.ammo[ BG_FindAmmoForWeapon(item->giTag)])
		{
			cg.ps.ammo[ BG_FindAmmoForWeapon(item->giTag)] = 1;
		}
	}
}
...
static void CG_TouchTriggerPrediction(void)
{
	...
	if (cg.ps.stats[STAT_HEALTH] <= 0)
	{
		return;
	}

	spectator = ((cg.ps.pm_type == PM_SPECTATOR) || (cg.ps.pm_flags & PMF_LIMBO));   // JPW NERVE

	if (cg.ps.pm_type != PM_NORMAL && !spectator)
	{
		return;
	}
	...
	for (i = 0 ; i < cg_numTriggerEntities ; i++)
	{
		...
		if (ent->eType == ET_ITEM && !spectator && (cg.ps.groundEntityNum == ENTITYNUM_WORLD))
		{
			CG_TouchItem(cent);
			continue;
		}
		   ...
		   )
		{
			...
			VectorAdd(cg.ps.origin, cg_pmove.mins, pmins);
			VectorAdd(cg.ps.origin, cg_pmove.maxs, pmaxs);
...
Generates cg.ps for the current cg.time
cg.ps is guaranteed to be valid after exiting.
...
void CG_PredictPlayerState(void)
{
	...
	else
	{
		...
		cg_pmove.recordMode = tjg_recordMode.integer;
		cg_pmove.disabled_ob = tjg_disabled_ob.integer;
		cg_pmove.isTimerun = isTimerun.integer;

		cg_pmove.noclipscale = tj_noclipscale.integer;
		...
		for (cmdNum = current - CMD_BACKUP + 1 ; cmdNum <= current ; cmdNum++)
		{
			...
			// Dini, the bugbug might be here, seeing as its to do with the view (angles!):P
			// Yes, the bugbug seems to be caused by this, from what I can see it only updates
			// view angles according to some pmove stuff, thats not really wanted imo!
			// No1 wants their angle changed by this even if it theoretically does some "prediction fix", it fails.
			...
			if (cg_pmove.cmd.serverTime <= cg.ps.commandTime)
			{
				continue;
			}
			...
			// we want to compare
			if (cg.ps.commandTime == oldPlayerState.commandTime)
			{
				vec3_t	delta;
				float	len;
				...
				{
					vec3_t	adjusted;
					CG_AdjustPositionForMover(cg.ps.origin, cg.ps.groundEntityNum, cg.physicsTime, cg.oldTime, adjusted, deltaAngles);
					// RF, add the deltaAngles (fixes jittery view while riding trains)
					// ydnar: only do this if player is prone or using set mortar
					if ((cg.ps.eFlags & EF_PRONE) || cg.weaponSelect == WP_MORTAR_SET)
						cg.ps.delta_angles[YAW] += ANGLE2SHORT(deltaAngles[YAW]);
		...
		if (!cg.showGameView)
		{
			...
			CG_AdjustPositionForMover(cg.ps.origin, cg.ps.groundEntityNum, cg.physicsTime, cg.time, cg.ps.origin, deltaAngles);
		}

		// fire events and other transition triggered things
		CG_TransitionPlayerState(&cg.ps, &oldPlayerState);
