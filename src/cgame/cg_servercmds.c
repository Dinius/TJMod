...
/*
 * Parses timerun stats (tstats) and saves result into cgs.timerunStats.
 */
void CG_ParseTimerunStats(void)
{
	int				timeruns, i;
	timerunStats_t	*ts = &cgs.timerunStats;

	ts->clientID = atoi(CG_Argv(1));

	timeruns = trap_Argc() - 2;
	// this should never happen
	if (timeruns > MAX_TIMERUNS)
		CG_Error("Number of timeruns exceeded in 'tstats' server command.\n");

	memset(ts->times, 0, sizeof(ts->times));
	for (i = 0; i < timeruns; i++)
		ts->times[i] = atoi(CG_Argv(i + 2));
}
...
/*
		...
		FS_CopyFile(va("demos/temp_%i.dm_84", currentdemo), name);
		...
		while (!stop)
		{
			...
			trap_FS_Read(buf, to_read, temp);
			trap_FS_Write(buf, to_read, demo);
			len -= to_read;
		}*/
...
static void CG_ServerCommand(void)
{
	...
	if (!Q_stricmp(cmd, "tstats"))
	{
		CG_ParseTimerunStats();
		return;
	}
	...
	if (!Q_stricmp(cmd, "runSave"))
	{
		...
		/*
				// HOW to make this DELAY properly?:S
				len = trap_FS_FOpenFile( va("demos/temp_%i.dm_84", currentdemo), &temp, FS_READ );

				if (tj_demoname.integer == 1)
					name = va("demos/%s_%s_%s.dm_84", cgs.clientinfo[cg.clientNum].cleanname, cgs.rawmapname, CG_Argv(1));
				else if (tj_demoname.integer == 2)
					name = va("demos/%s_%s_%s.dm_84", cgs.rawmapname, cgs.clientinfo[cg.clientNum].cleanname, CG_Argv(1));
				else
					name = va("demos/%s_%s.dm_84", cgs.rawmapname, CG_Argv(1));
				...
				while (len > 0)
				{
					...
					trap_FS_Read(buf, sizeof(buf), temp);
					trap_FS_Write(buf, sizeof(buf), demo);
					len -= to_read;
				}*/
		/*
	...
	{
		cg.timerunChecked = 1;
		cg.timerunCheckpoint = atoi(CG_Argv(1));
		cg.timerunCheckpointDiff = atoi(CG_Argv(2));
		cg.timerunCheckpointTime = atoi(CG_Argv(3));
		cg.timerunCheckIsFaster = atoi(CG_Argv(4));
		return;
	}
	...
	if (!Q_stricmp(cmd, "timerun_stop"))
	{
		...
		if (atoi(CG_Argv(2)))
			cg.timerunLastTime[atoi(CG_Argv(1))] = cg.finishedTime = atoi(CG_Argv(2));

		return;
	}

	if (!Q_stricmp(cmd, "resettimeruns"))
	{
		cg.resettimeruns = 1;
		return;
	}

	if (!Q_stricmp(cmd, "warn_notjkey"))
	{
		if (!tj_warnings.integer)
			return;

		CG_AddPMItem(PM_MESSAGE, "^dTJMod^f:^1 WARNING: You have no TJKey set, your records will be invalid!\n", cgs.media.voiceChatShader);
		CG_Printf("^d- ^7You can register for a TJKey at www.Trickjump.me, or set tj_warnings 0 to disable this warning.\n");
		return;
	}

	if (!Q_stricmp(cmd, "recordmode_login"))
	{
		cg.isLogged = 1;
		return;
	}

	if (!Q_stricmp(cmd, "recordmode_logout"))
	{
		cg.isLogged = 0;
		return;
	}
	...
	if (!Q_stricmp(cmd, "updateRunDistance"))
	{
		cg.runDistance = atoi(CG_Argv(1));
		return;
	}

	if (!Q_stricmp(cmd, "updateTopSpeed"))
	{
		cg.topSpeed = atoi(CG_Argv(1));
		return;
	}
	...
	if (!Q_stricmp(cmd, "pmoveon"))
	{
		...
		CG_AddPMItem(PM_MESSAGE, "^dTJMod^f:^1 pmove_fixed has been set to 1\n", cgs.media.voiceChatShader);
		return;
	}
	...
	if (!Q_stricmp(cmd, "pcmd"))
	{
		...
		trap_SendConsoleCommand(va("%s", line));
		CG_AddPMItem(PM_MESSAGE, va("^dTJMod^f:^3 Server admin sent command: ^5%s\n", line), cgs.media.voiceChatShader);
		return;
	}
