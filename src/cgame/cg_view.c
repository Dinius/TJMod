...
void CG_OffsetThirdPersonView(void)
{
	...
	cg.refdef_current->vieworg[2] += cg.ps.viewheight;
	...
	if (cg.ps.stats[STAT_HEALTH] <= 0)
	{
		...
		if (cg.snap->ps.viewlocked != 7)
		{
			...
			focusAngles[YAW] = SHORT2ANGLE(cg.ps.stats[STAT_DEAD_YAW]);
			cg.refdefViewAngles[YAW] = SHORT2ANGLE(cg.ps.stats[STAT_DEAD_YAW]);
		}
	}
	...
	if (cg_thirdPerson.integer == 2)
		VectorCopy(cg.ps.origin, focusPoint);
	else
	...
	CG_Trace(&trace, cg.refdef_current->vieworg, mins, maxs, view, cg.ps.clientNum, MASK_SOLID);
...
static void CG_OffsetFirstPersonView(void)
{
	...
	VectorCopy(cg.ps.velocity, predictedVelocity);
	...
	if (cg.ps.leanf != 0)
	{
		//add leaning offset
		vec3_t	right;
		cg.refdefViewAngles[2] += cg.ps.leanf / 2.0f;
		AngleVectors(cg.refdefViewAngles, NULL, right, NULL);
		VectorMA(cg.refdef_current->vieworg, cg.ps.leanf, right, cg.refdef_current->vieworg);
	}
...
static int CG_CalcFov(void)
{
	...
	if (cg.ps.stats[STAT_HEALTH] <= 0 && !(cg.snap->ps.pm_flags & PMF_FOLLOW))
	{
		cg.zoomedBinoc = qfalse;
		cg.zoomTime = 0;
		cg.zoomval = 0;
	...
	if (cg.ps.pm_type == PM_INTERMISSION)
	{
...
int CG_CalcViewValues(void)
{
	...
	if (!cg.showGameView)
	{
		...
		else if (ps->viewlocked)
		{
			...
			AngleVectors(cg.refdefViewAngles, fwd, NULL, NULL);
			if (cg.ps.eFlags & EF_AAGUN_ACTIVE)
			{
...
void CG_DrawSkyBoxPortal(qboolean fLocalView)
{
	...
	// Arnout: mg42 zoom
	if (cg.snap->ps.persistant[PERS_HWEAPON_USE] || cg.ps.pm_flags & PMF_PRONE_BIPOD)
	{
		fov_x = 55;
	}
	...
	if (fLocalView)
	{
		...
		if (BG_PlayerMounted(cg.snap->ps.eFlags) || cg.ps.weapon == WP_MOBILE_MG42_SET)
		{
			fov_x = 55;
		}
