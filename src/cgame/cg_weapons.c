...
{
	if (cg.ps.pm_type >= PM_DEAD)
		return;

	if (cg.pmext.weapAnimTimer > 0)
		return;

	if (cg.ps.weapon == WP_NONE)
		return;

	cg.ps.weapAnim = ((cg.ps.weapAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) | anim;
}
...
static void CG_ContinueWeaponAnim(int anim)
{
	...
	if ((cg.ps.weapAnim & ~ANIM_TOGGLEBIT) == anim)
	{
		return;
	}
	if (cg.pmext.weapAnimTimer > 0)
...
static void CG_CalculateWeaponPosition(vec3_t origin, vec3_t angles)
{
	...
	if (cg.ps.eFlags & EF_MOUNTEDTANK)
	{
		angles[PITCH] = cg.refdefViewAngles[PITCH] / 1.2;
	}

	if (!cg.renderingThirdPerson &&
			(cg.ps.weapon == WP_MORTAR_SET || cg.ps.weapon == WP_MOBILE_MG42_SET) &&
			cg.ps.weaponstate != WEAPON_RAISING)
	{
	...
	if (cg.ps.leanf != 0)
	{
		...
		VectorMA(origin, -cg.ps.leanf / 4.0f, right, origin);
	}
	...
	// idle drift
	if ((!(cg.ps.eFlags & EF_MOUNTEDTANK)) && (cg.ps.weapon != WP_MORTAR_SET) && (cg.ps.weapon != WP_MOBILE_MG42_SET))
	{
...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent)
{
	...
	if (BG_IsAkimboWeapon(weaponNum))
	{
		if (isPlayer)
		{
			akimboFire = BG_AkimboFireSequence(weaponNum, cg.ps.ammoclip[BG_FindClipForWeapon(weaponNum)], cg.ps.ammoclip[BG_FindClipForWeapon(BG_AkimboSidearm(weaponNum))]);
		}
		else if (ps)
		{
	...
	if (ps && !cg.renderingThirdPerson && cg.ps.weapon == WP_MORTAR_SET && cg.ps.weaponstate != WEAPON_RAISING)
	{
		vec3_t angles;
	...
	if (ps)
	{
		...
		for (i = W_PART_1; i < W_MAX_PARTS; i++)
		{
			...
			{
				if (ps && !cg.renderingThirdPerson && cg.ps.weaponstate != WEAPON_RAISING)
				{
					continue;
				}
			}
...
void CG_AddViewWeapon(playerState_t *ps)
{
	...
	if ((!cg_drawGun.integer))
	{
		...
		//bani - #589
		if (cg.ps.eFlags & EF_FIRING && !(cg.ps.eFlags & (EF_MG42_ACTIVE | EF_MOUNTEDTANK)))
		{
...
static qboolean CG_WeaponHasAmmo(int i)
{
	...
	if (!(cg.ps.ammo[BG_FindAmmoForWeapon(i)]) &&
			!(cg.ps.ammoclip[BG_FindClipForWeapon(i)]))
	{
		return qfalse;
	}
...
qboolean CG_WeaponSelectable(int i)
{
	...
	if (BG_PlayerMounted(cg.ps.eFlags))
	{
		return qfalse;
	}

	// check for weapon
	if (!(COM_BitCheck(cg.ps.weapons, i)))
	{
		return qfalse;
	}
...
void CG_AltWeapon_f(void)
{
	...
	{
		trap_SendConsoleCommand("panzeralt\n");
		return;
	}
	...
	if (cg.weaponSelect == WP_MORTAR)
	{
		...
		if (cg.ps.groundEntityNum == ENTITYNUM_NONE)
			return;
		if (!cg.ps.ammoclip[WP_MORTAR])
			return;

		if (cg.ps.eFlags & EF_PRONE)
		{
			return;
		}
...
void CG_OutOfAmmoChange(qboolean allowforceswitch)
{
	...
	if (cg.weaponSelect == WP_PLIERS || (cg.weaponSelect == WP_SATCHEL_DET && cg.ps.ammo[WP_SATCHEL_DET]))
	{
		return;
	}
	...
	if (allowforceswitch)
	{
		if (cg.weaponSelect == WP_SMOKE_BOMB)
		{
			if (CG_WeaponSelectable(WP_LUGER))
			{
				...
				CG_FinishWeaponChange(cg.ps.weapon, WP_LUGER);
				return;
			}
			else if (CG_WeaponSelectable(WP_COLT))
			{
				cg.weaponSelect = WP_COLT;
				CG_FinishWeaponChange(cg.ps.weapon, WP_COLT);
				return;
			}
		}
		else if (cg.weaponSelect == WP_LANDMINE)
		{
			if (CG_WeaponSelectable(WP_PLIERS))
			{
				...
				CG_FinishWeaponChange(cg.ps.weapon, WP_PLIERS);
				return;
			}
		}
		...
		if (cg.weaponSelect == WP_PANZERFAUST)
		{
			...
			for (i = 0; i < MAX_WEAPS_IN_BANK_MP; i++)
			{
				...
				if (weapBanksMultiPlayer[3][i] != WP_PANZERFAUST && CG_WeaponSelectable(weapBanksMultiPlayer[3][i]))   // find a smg
				{
					...
					CG_FinishWeaponChange(cg.ps.weapon, cg.weaponSelect);
					return;
				}
			} // b021
			for (i = 0; i < MAX_WEAPS_IN_BANK_MP; i++)
		...
		if (cg.weaponSelect >= WP_BEGINSECONDARY && cg.weaponSelect <= WP_LASTSECONDARY)
		{
			...
			{
				CG_FinishWeaponChange(cg.ps.weapon, cg.weaponSelect);//----(SA)
				return;
			}
		}
...
void CG_FireWeapon(centity_t *cent)
{
	...
	if (ent->weapon == WP_MORTAR_SET)
	{
		if (ent->clientNum == cg.snap->ps.clientNum)
		{
			...
			cg.mortarFireAngles[PITCH] = cg.ps.viewangles[PITCH];
			cg.mortarFireAngles[YAW] = cg.ps.viewangles[YAW];
		}
	}
	...
	if (!(cent->currentState.eFlags & EF_ZOOMING))   // JPW NERVE -- don't play sounds or eject brass if zoomed in
	{
		...
		// do brass ejection
		if (weap->ejectBrassFunc && cg_brassTime.integer > 0 && tj_brassflash.integer)
		{
...
void CG_Bullet(vec3_t end, int sourceEntityNum, vec3_t normal, qboolean flesh, int fleshEntityNum, int otherEntNum2, float waterfraction, int seed)
{
	...
	if (cgs.antilag && otherEntNum2 == cg.snap->ps.clientNum && cg_entities[otherEntNum2].currentState.eFlags & EF_MG42_ACTIVE)
	{
		...
		AngleVectors(cg.ps.viewangles, forward, right, up);
