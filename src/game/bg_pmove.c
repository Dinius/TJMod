/*
 * Both games player movement code.
 *
 * Takes a playerstate and a usercmd as input and returns a modifed
 * playerstate.
 */
...
==================
*/
// Dini, Note, without this theres no friction,
// aka cpm style ice, etc.
...
==============
*/
static void PM_AccelerateIce(vec3_t wishdir, float wishspeed, float accel)
{
	int				i;
	float			addspeed, accelspeed, currentspeed;
	usercmd_t		cmd;
	...
	if (pm->physics >= PHYSICS_NOWALLSTRAFE)
		VectorCopy(pm->ps->velocity, pm->oldvel);
	...
	if (!smove && fmove > 0)
	{
		...
		// 0.16, 999999, 16.0, 0.0666
		float bunnyaccel = 0.16f; // (0.42 0.1593f) Default: 0.1585f how fast you accelerate after reaching pm_maxspeed
		// (it gets harder as you near bunnytopspeed)
		float bunnytopspeed = 999999; // (0.42: 925) soft speed limit (can get faster with rjs and on ramps)
		float turnaccel = 16.0f;    // (0.42: 9.0) Default: 7 max sharpness of turns
		float backtosideratio = 0.5f; // (0.42: 0.8) Default: 0.8f lower values make it easier to change direction without
		// losing speed; the drawback is "understeering" in sharp turns
		static vec_t speed, oxy;
		vec3_t vel;

		if (!wishspeed)
			return;

		speed = oxy = sqrt(pm->ps->velocity[0] * pm->ps->velocity[0] + pm->ps->velocity[1] * pm->ps->velocity[1]);
		...
		VectorMA(vel, accelspeed, acceldir, vel);

		if (sqrt(vel[0] * vel[0] + vel[1] * vel[1]) > oxy)
		{
			vel[0] = pm->ps->velocity[0];
			vel[1] = pm->ps->velocity[1];
		}

		if (sqrt(vel[2] * vel[2]) > sqrt(pm->ps->velocity[2] * pm->ps->velocity[2]))
			vel[2] = pm->ps->velocity[2] * 1.001;
		else
			vel[2] = pm->ps->velocity[2];
	...
	// GOODBYE WALLSTRAFE!
	if (pm->physics >= PHYSICS_NOWALLSTRAFE && pm->xyspeed > 352)
	{
		if (pm->physics >= PHYSICS_NOWALLSTRAFE && PM_TheoreticalSlideMove(qfalse))
		{
			pm->ps->velocity[0] = pm->oldvel[0];
			pm->ps->velocity[1] = pm->oldvel[1];
		}
	}
}
...
/*
	...
	float backtosideratio = 0.12f; // (0.42: 0.8) Default: 0.8f lower values make it easier to change direction without
	// losing speed; the drawback is "understeering" in sharp turns
	...
	{
		float accelspeed = curspeed + airforwardaccel * MAXSPEED * pml.frametime;
		if( accelspeed < wishspeed )
			wishspeed = accelspeed;
	}
	else
	{
		float f = ( bunnytopspeed - curspeed ) / ( bunnytopspeed - MAXSPEED );
		if( f < 0 )
			f = 0;
		wishspeed = max( curspeed, MAXSPEED ) + bunnyaccel * f * MAXSPEED * pml.frametime;
	}

*/


static void PM_AccelerateW(vec3_t wishdir, float wishspeed, float accel)
{
	vec3_t curvel, wishvel, acceldir, curdir;
	float addspeed, accelspeed, curspeed;
	float dot;
	// air movement parameters:
	float bunnyaccel = 0.18f; // (0.42 0.1593f) Default: 0.1585f how fast you accelerate after reaching pm_maxspeed
	// (it gets harder as you near bunnytopspeed)
	float bunnytopspeed = 999999; // (0.42: 925) soft speed limit (can get faster with rjs and on ramps)
	float turnaccel = 6.66f;    // (0.42: 9.0) Default: 7 max sharpness of turns
	float backtosideratio = 0.0666f; // (0.42: 0.8) Default: 0.8f lower values make it easier to change direction without
	// losing speed; the drawback is "understeering" in sharp turns
...
static float PM_CmdScale(usercmd_t *cmd, qboolean horizontalOnly)
{
	...
	// ignore horizontalOnly flag if we use the original physics
	if (pm->physics < PHYSICS_FIXED_UPMOVE_BUG)
...
static qboolean PM_CheckJump(void)
{
	...
	// fix for #166
	if (pm->physics < PHYSICS_FLAT && pm->cmd.serverTime - pm->pmext->jumpTime < 850)
		return qfalse;

	// AP prone insta-jump bugfix
	// This is a mix of lame and fail
	//if (pm->physics > PHYSICS_FLAT && pm->cmd.serverTime - pm->pmext->proneJumpDelay < 400)
	//return qfalse;
	...
	if (pm->physics >= PHYSICS_DOUBLEJUMP && pm->ps->velocity[2] > 0)
	...
	if (pm->ps->eFlags & EF_PRONE)
	{
				...
				((pm->cmd.doubleTap == DT_BACK || pm->cmd.upmove > 10 || pm->cmd.wbuttons & WBUTTON_PRONE) && pm->cmd.serverTime - pm->pmext->proneTime > 750))
		{
			...
			if (!trace.allsolid)
			{
				...
				// Dini, to fix ""bug"" in AP with proning going instajump this
				// stuff needs to be re-added so it will again be a delay in checkjump.
				//pm->pmext->proneJumpDelay = pm->cmd.serverTime;
...
static void PM_WaterJumpMove(void)
{
	...
	pm->ps->velocity[2] -= 800 * pml.frametime;
...
vec_t VectorLength2(const vec3_t v)
{
...
static void PM_AirMove(void)
{
	...
	if (pm->physics < PHYSICS_AIRCONTROL || (pm->isTimerun && pm->recordMode && pm->isLogged && (!pm->timerunActive || pm->timerunStartTime > pm->cmd.serverTime)))
	...
	else
	{
		...
		else if (!smove && fmove > 0 && pm->xyspeed >= 352)
			PM_AccelerateW(wishdir, wishspeed, pm_airaccelerate);
		else
			PM_Accelerate(wishdir, wishspeed, pm_airaccelerate);
	}
	...
	if (pm->physics >= PHYSICS_NOWALLSTRAFE && pm->xyspeed > 352)
	{
		if (PM_TheoreticalSlideMove(qtrue) && pm->groundTrace.fraction < 1.1f)
		{
			if (VectorLength2(pm->ps->velocity) > VectorLength2(pm->oldvel))
			{
				pm->ps->velocity[0] = pm->oldvel[0];
				pm->ps->velocity[1] = pm->oldvel[1];
			}
...
static void PM_WalkMove(void)
{
	...
	// Icecontrol.. I'm so good at coming up with names for things, I should start working as a name-commer-upper-with
	if (pm->physics >= PHYSICS_ICECONTROL && pm->groundTrace.surfaceFlags & SURF_SLICK)
		PM_AccelerateIce(wishdir, wishspeed, accelerate * 9);
	else
	...
	// slide along the ground plane (ABOVE vec.nor.\sca.)
	PM_ClipVelocity(pm->ps->velocity, pm->groundTrace.plane.normal, pm->ps->velocity, OVERCLIP);
...
static void PM_CrashLand(void)
{
	...
	if (!(pm->groundTrace.surfaceFlags & SURF_NODAMAGE) && pm->physics < PHYSICS_FLAT)
	{
		if (pm->debugLevel)
		...
		*/
		...
		// wrong, otherwise.)
		// setup - don't clear velocity on "nodamage" surface
		if (delta > 38.75)
			VectorClear(pm->ps->velocity);
...
=============
*/
// Dini, Note, without (most of) this stuff, game would behave
// as if your current Z - position = the ground.
static void PM_GroundTrace(void)
{
	vec3_t		point;
	trace_t		trace;
	qboolean	noOB = qfalse;

	if (pm->disabled_ob || pm->recordMode || pm->physics >= PHYSICS_DISABLED_OB)
		noOB = qtrue;
	...
	if (pm->ps->groundEntityNum == ENTITYNUM_NONE)
	{
		...
		// Dini, Note, remove this => everything is cushion
		PM_CrashLand();
...
// Dini, removed spread for sten and garand\k43 (not when scoped!)
...
static void PM_Weapon(void)
{
	...
	if (pm->physics == PHYSICS_NORMAL && (!(pm->ps->eFlags & EF_PRONE) && (pm->groundTrace.surfaceFlags & SURF_SLICK)))
	{
...
void PmoveSingle(pmove_t *pmove)
{
	...
	{
		if (COM_BitCheck(pm->ps->weapons, WP_BINOCULARS))  	// player got binoculars
		{
	...
	// these three items were previously in pml too, so clear them too
	...
	if (pm->ps->weapon == WP_MORTAR_SET && pm->ps->pm_type == PM_NORMAL)
	{
		...
		// handle movement (gravity)
		{
			...
			endVelocity[2] -= 800 * pml.frametime;
...
int Pmove(pmove_t *pmove)
{
	...
	if (pmove->ps->curWeapHeat > 999)
		pmove->ps->curWeapHeat = 999;
