...
#define	GAME_VERSION	"tjmod"
#define GV_PREFIX		"^dTJMod^f:^7"
#define	TJMOD_VERSION	"2.0.0 ^1BETA^7"
//"2.0.0 BETA"

#if defined(_DEBUG)
#define	GAME_VERSION_DATED			GAME_VERSION
#else
#define	GAME_VERSION_DATED			(GAME_VERSION ", " TJMOD_VERSION)
#endif
...
#define CGF_RESETVELOCITY	0x20
#define CGF_AUTOLOGIN		0x40
#define CGF_PMOVEFIXED		0x80
#define CGF_LOADVIEWANGLES	0x100
#define CGF_CPMTIMES		0x200
#define CGF_BPTIMES			0x400
...
#define CS_TIMERUNS						( CS_STRINGS +				MAX_CSSTRINGS				)
#define CS_MAX							( CS_STRINGS +				MAX_TIMERUNS				)
...
// pmove physics
// The highest number (the one at bottom) is generally the requirement for official _AP_ records.
typedef enum
{
	PHYSICS_NORMAL,				// classic ET physics
	PHYSICS_FLAT,				// + flat jumping + no fall damage
	PHYSICS_DOUBLEJUMP,			// + doublejump (as in on ramps and alike, not in-air)
	PHYSICS_AIRCONTROL,			// + aircontrol (turning with a\d and w)
	PHYSICS_FIXED_UPMOVE_BUG,	// + without upmove bug
	PHYSICS_DISABLED_OB,		// + without overbounce
	PHYSICS_ICECONTROL,			// + icecontrol (more "speed" + "aircontrol" on ice)
	PHYSICS_NOWALLSTRAFE		// + "disables" wallstrafe (no speed gain \ very little gain.) No additional "loss" though :)
} physics_t;

#define MAX_ADMIN_LEVELS 4096
...
#define EF_FAKEBMODEL		0x00010000
