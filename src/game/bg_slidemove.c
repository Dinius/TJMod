		...
		for (i = 0 ; i < numplanes ; i++)
		{
			if (DotProduct(trace.plane.normal, planes[i]) > 0.99)
			{
				...
				else
				{
					...
					// Dini, DJ, Keeps speed after "being put on top"
					// DINI, FIXME, ONLY FOR CASES WHERE DJ HAPPENS :?
					VectorAdd(trace.plane.normal, pm->ps->velocity, pm->ps->velocity);
				}
				break;
			}
...
// Doesn't modify your actual speed
qboolean PM_TheoreticalSlideMove(qboolean gravity)
{
	int			bumpcount, numbumps, extrabumps;
	vec3_t		dir;
	float		d;
	...
	// Replaces actual values, so it has no real effect => theoretical
	...
	if (gravity)
	{
		VectorCopy(vel, endVelocity);
		endVelocity[2] -= 800 * pml.frametime;
		vel[2] = (vel[2] + endVelocity[2]) * 0.5;
		primal_velocity[2] = endVelocity[2];
		if (pm->groundPlane)
		{
			// slide along the ground plane
			PM_ClipVelocity(vel, pm->groundTrace.plane.normal,	vel, OVERCLIP);
		}
	}
	else
	{
	...
	// never turn against original velocity
	VectorNormalize2(vel, planes[numplanes]);
	numplanes++;
	...
	for (bumpcount = 0 ; bumpcount < numbumps ; bumpcount++)
	{
		...
		{
			// entity is completely trapped in another solid
			vel[2] = 0;	// don't build up falling damage, but allow sideways acceleration
			return qtrue;
		}
		...
		for (i = 0 ; i < numplanes ; i++)
		{
			...
			// slide along the plane
			PM_ClipVelocity(vel, planes[i], clipVelocity, OVERCLIP);
	...
	if (gravity)
	{
		VectorCopy(endVelocity, vel);
	}

	// don't change velocity if in a timer (FIXME: is this correct?)
	if (pm->ps->pm_time)
	{
		VectorCopy(primal_velocity, vel);
	}
...
{
	// Dini, fixme, this seems ok but it's ... dumb
	if (1 == 1 && pm->ps->velocity[2] < 100 && pm->ps->velocity > 0)
	{
		...
		if (PM_SlideMove(gravity) == 0)
		{
			...
			// we can step down
			if (trace.fraction > 0.01f && trace.fraction < 1.0f && !trace.allsolid && pm->groundPlane != qfalse)
			{
				if (pm->debugLevel)
