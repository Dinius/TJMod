/*
 * Processing of one player frame.
 */
...
void SpectatorThink(gentity_t *ent, usercmd_t *ucmd)
{
	...
	if (client->pers.nofatigue && tjg_Nofatigue.integer)
...
qboolean ClientInactivityTimer(gclient_t *client)
{
	...
	else if (!client->pers.localClient)
	{
		if (level.time > client->inactivityTime && client->inactivityWarning)
		{
			client->inactivityWarning = qfalse;
			client->inactivityTime = level.time + 60 * 1000;
			// Dini, FIXME, put to spec, dont kick!
			...
			AP(va("cpm \"" GV_PREFIX " %s ^7removed from teams due to inactivity! ^z(%i seconds) \n\"", client->pers.netname, g_inactivity.integer));
			trap_SendServerCommand(client - level.clients, "timerun_stop");
...
void ClientThink_real(gentity_t *ent)
{
	...
	if (client->timerunActive && client->ps.ping > 400)
	{
		...
		CPx(ent - g_entities, "cpm \"^dTJMod^f: ^1Timerun stopped, too high ping detected!\n\"");
		trap_SendServerCommand(ent - g_entities, "timerun_stop");
	...
	client->ps.gravity = GAME_GRAVITY;
	client->ps.speed = GAME_SPEED;
	...
	pm.recordMode = tjg_recordMode.integer;
	pm.disabled_ob = tjg_disabled_ob.integer;
	pm.isTimerun = isTimerun.integer;

	pm.noclipscale = client->pers.noclipscale;

	pm.timerunActive = client->timerunActive;
	pm.timerunStartTime = client->timerunStartTime + 500; // more\less? hmm
	...
	if (client->sess.logged && !client->pers.pmoveFixed)
	{
		CP("cpm \"" GV_PREFIX " You were removed from teams because you can not use pmove_fixed 0 while logged in.\n\"");
...
void ClientEndFrame(gentity_t *ent)
{
	...
	*/
	...
	// Dini, Timerun stats related
	curSpeed = sqrt(ent->client->ps.velocity[0] * ent->client->ps.velocity[0] + ent->client->ps.velocity[1] * ent->client->ps.velocity[1]);

	if (level.time > ent->client->nextTime && ent->client->timerunActive)
	{
		if (curSpeed > ent->client->topSpeed)
		{
			ent->client->topSpeed = curSpeed;
			trap_SendServerCommand(ent - g_entities, va("updateTopSpeed %0.f", ent->client->topSpeed));
		}

		if (!ent->client->lastOrigin)
			VectorCopy(ent->client->ps.origin, ent->client->lastOrigin);

		distance = Distance(ent->client->ps.origin, ent->client->lastOrigin);
		if (distance > 0)
		{
			ent->client->runDistance += distance;
			VectorCopy(ent->client->ps.origin, ent->client->lastOrigin);
			trap_SendServerCommand(ent - g_entities, va("updateRunDistance %0.f", ent->client->runDistance));
		}

		ent->client->nextTime = level.time + 500;
	}
	else if (!ent->client->timerunActive)
	{
		ent->client->ps.curWeapHeat = 0;
		ent->client->topSpeed = 0;
		ent->client->runDistance = 0;
		VectorCopy(ent->client->ps.origin, ent->client->lastOrigin);
		ent->client->nextTime = level.time - 1;
	}
