/*
 * Functions to handle player's connection, userinfo, spawning and others.
 */
...
void limbo(gentity_t *ent, qboolean makeCorpse)
{
	...
	if (!(ent->client->ps.pm_flags & PMF_LIMBO))
	{
		...
		Screw corpses, at least for now */
		trap_UnlinkEntity(ent);
		...
		/* Don't need weapons lying around either
...
===========
*/
// Dini, Note, This sets \ gives players weapons.
void SetWolfSpawnWeapons(gclient_t *client)
{
	...
	// Knife will always be given.
	AddWeaponToPlayer(client, WP_KNIFE, 1, 0, qtrue);

	client->ps.weaponstate = WEAPON_READY;

	// Dini, dont add below weapons until TJMod section specific if tjg_weapons is disabled
	if (tjg_weapons.integer && !tjg_recordMode.integer)
	{
		...
		if (pc == PC_ENGINEER)
		{
			...
			if (g_knifeonly.integer != 1)
			{
				if (client->sess.sessionTeam == TEAM_AXIS)
				{
					...
					if (!level.noExplosives || !tjg_mapExplosiveToggle.integer)
					{
		...
		if (g_knifeonly.integer != 1)
		{
			...
			if (pc == PC_SOLDIER && (!level.noExplosives || !tjg_mapExplosiveToggle.integer))
			{
				if (client->sess.sessionTeam == TEAM_AXIS)
				{
					AddWeaponToPlayer(client, WP_GRENADE_LAUNCHER,  0, 4, qfalse);
			...
			if (pc == PC_COVERTOPS && (!level.noExplosives || !tjg_mapExplosiveToggle.integer))
			{
				if (client->sess.sessionTeam == TEAM_AXIS)
				{
	...
	// =========================================================
	// START TJMOD WEAPONS
	// Dini, TJMod Weapons added by default, and extra

	// Everyone gets binocs
	AddWeaponToPlayer(client, WP_BINOCULARS, 1, 0, qfalse);

	// Give all medics adren
	if (pc == PC_MEDIC)
	{
		AddWeaponToPlayer(client, WP_MEDIC_ADRENALINE, 0, 1, qfalse);
	...
	if (level.rocketRun)
	{
		AddWeaponToPlayer(client, WP_PANZERFAUST, 0, level.rocketRun, qtrue);
	}
	else if (tjg_recordMode.integer)
	{
		if (pc == PC_ENGINEER)
		{
			AddWeaponToPlayer(client, WP_PLIERS, 0, 1, qfalse);
...
/*
 * Cleans client name from invalid characters. Returns qfalse if client name
 * was too short.
 */
static qboolean ClientCleanName(const char *in, char *out, int outSize)
{
	...
	*p = '\0';
	...
	while (1)
	{
		...
		// don't allow `=' operator because it could break Quoted-Printable
...
void ClientUserinfoChanged(int clientNum)
{
	...
	/*
		Removed due to lazyness fix-versa
	...
	client->pers.resetVelocityOnLoad = client->pers.clientFlags & CGF_RESETVELOCITY;
	client->pers.loadViewAngles = client->pers.clientFlags & CGF_LOADVIEWANGLES;
	client->pers.cpmTimes = client->pers.clientFlags & CGF_CPMTIMES;
	client->pers.bpTimes = client->pers.clientFlags & CGF_BPTIMES;


	for (i = 0; i < level.admin.curlev; i++)
	{
		if (!strcmp(client->pers.pass, level.admin.pass[i]))
		{
			client->pers.admin = level.admin.level[i];
			continue;
		}
	}

	if (!client->pers.autoLogin && (client->pers.clientFlags & CGF_AUTOLOGIN) && !client->sess.logged && level.isTimerun)
		G_Login(ent);
	...
	if (!ClientIsFlooding(ent))
	{
		...
		// Too short name?
		if (!ClientCleanName(s, client->pers.netname, sizeof(client->pers.netname)))
			trap_DropClient(clientNum, "Name too short.", 0);

		// print the namechange?
	...
	s = va("n\\%s\\t\\%i\\c\\%i\\r\\%i\\m\\%s\\s\\%s\\dn\\%s\\dr\\%i\\w\\%i\\lw\\%i\\sw\\%i\\mu\\%i\\ref\\%i\\pm\\%i\\fps\\%i\\tjk\\%i\\tbt\\%i\\h\\%i",
		   client->pers.netname,
...
// Dini, IP conncetions limit from etpub
...
char *ClientConnect(int clientNum, qboolean firstTime, qboolean isBot)
{
	...
	// FIXME save already parsed IP so it don't need to be parsed again
	...
	for (i = 0; i < level.numConnectedClients; i++)
	{
		...
		ip2 = GetParsedIP(value); // this should be save since it was already once successufully parsed
		if (strcmp(ip, ip2) == 0)
		{
			conn_per_ip++;
		}
	}
	if (conn_per_ip > tjg_maxConnections.integer)
	{
	...
	if (!strcmp(Info_ValueForKey(userinfo, "ip"), "localhost"))
	{
	...
	if (firstTime)
	{
		...
		client->sess.referee = (client->pers.localClient) ? RL_REFEREE : RL_NONE;
	...
	// FIXME these should be saved in session data
	client->sess.allowNoclip = qtrue;
	client->sess.allowSL = qtrue;		// allow save/load
	client->sess.allowGoto = qtrue;
	client->sess.allowIwant = qtrue;
	client->warnCount = 0;
...
void ClientDisconnect(int clientNum)
{
	...
	// remove player from all ignore lists
	for (i = 0; i < MAX_CLIENTS; i++)
		COM_BitClear(level.clients[i].sess.ignoreClients, clientNum);

	// remove the client from all specInvited lists
	for (i = 0; i < level.numConnectedClients; i++)
