...
static const cmd_reference_t aCommandInfo[] =
{
	...
	{ "specunlock",		qtrue,	qfalse,	Cmd_SpecLock_f, ":^7 Unlocks a player from spectators" },
	{ "speclist",		qtrue,	qtrue,	Cmd_SpecList_f, ":^7 Lists specinvited players" },
...
{
	int i, idnum, max_rate = 100000, cnt = 0, tteam;
	...
	if (g_gamestate.integer == GS_PLAYING)
	{
		if (ent)
		{
			CP("print \"\n^3 ID^f : ^3Player                    Nudge  Rate  MaxPkts  Snaps FPS PMove IP\n\"");
			CP("print \"^f-----------------------------------------------------------------------------------^7\n\"");
		}
		else
		{
			G_Printf(" ID : Player                    Nudge  Rate  MaxPkts  Snaps  FPS   PMove   IP\n");
			G_Printf("----------------------------------------------------------------------------------n");
		}
	}
	else
	{
		if (ent)
		{
			CP("print \"\n^fID   Player                         Nudge  Rate  MaxPkts  Snaps\n\"");
			CP("print \"^f--------------------------------------------------------------^7\n\"");
		}
		else
		{
	...
	for (i = 0; i < level.numConnectedClients; i++)
	{
		...
		if (ent) CP(va("print \"%s%s%2d^f:%s %-26s^7%s  %i   %i   %i.%i.*.*  ^3%s\n\"", ready, tc, idnum, ((ref[0]) ? "^3" : "^7"), n2, rate, cl->pers.maxFPS, cl->pers.pmoveFixed ? 1 : 0, ip1, ip2, ref));
		else G_Printf("%s%s%2d: %-26s%s %s\n", ready, tc, idnum, n2, rate, ref);
