...
/*
 * Loads best timerun times from `timeruns/mapname.rec'. Such files need to be
 * supplied by external entity. This must be called after all timers related
...
void G_LoadTimerunsRecords(void)
{
	...
	Com_sprintf(filename, sizeof(filename), "timeruns/%s.rec", level.rawmapname);
	...
	// parse file content
	COM_BeginParseSession("G_LoadTimerunsRecords");
	while (qtrue)
	{
		...
		{
			G_Printf("G_LoadTimerunsRecords, Error (line %d): 'record' expected.\n",
		...
		if (strcmp(token, "{"))
		{
			G_Printf("G_LoadTimerunsRecords, Error (line %d): '{' expected.\n",
					 COM_GetCurrentParseLine());
			return;
		}
		...
		{
			token = COM_Parse(&data);
			if (!strcmp(token, "run_name"))
			{
				token = COM_Parse(&data);
				if (!token[0])
				{
					G_Printf("G_LoadTimerunsRecords, Error (line %d): Timerun name excepted,"
							 " end of script found.\n", COM_GetCurrentParseLine());
					return;
				}
				Q_strncpyz(timerunName, token, sizeof(timerunName));
			}
			else if (!strcmp(token, "time"))
			{
				...
				if (!token[0])
				{
					G_Printf("G_LoadTimerunsRecords, Error (line %d): Timerun time excepted,"
							 " end of script found.\n", COM_GetCurrentParseLine());
					return;
				}
				if (sscanf(token, "%d:%d.%d", &mins, &secs, &mil) != 3)
				{
					G_Printf("G_LoadTimerunsRecords, Error (line %d): Malformed time found.\n",
							 COM_GetCurrentParseLine());
					return;
				}
				timerunTime = mins * 60000 + secs * 1000 + mil;
			}
			else if (!strcmp(token, "player"))
			{
				...
				if (!token[0])
				{
					G_Printf("G_LoadTimerunsRecords, Error (line %d): Player name excepted,"
							 " end of script found.\n", COM_GetCurrentParseLine());
					return;
				}
				if (strlen(token) >= sizeof(timerunPlayer))
				{
					G_Printf("G_LoadTimerunsRecords, Error (line %d): Player name too long.\n",
							 COM_GetCurrentParseLine());
					return;
				}
				Q_strncpyz(timerunPlayer, token, sizeof(timerunPlayer));
			}
			else if (!strcmp(token, "}"))
				break;
			else
			{
				G_Printf("G_LoadTimerunsRecords, Error (line %d): 'run_name', 'time', 'player'"
						 " or '}' expected.\n", COM_GetCurrentParseLine());
				return;
			}
		}
		...
		if (!timerunName[0])
		{
			G_Printf("G_LoadTimerunsRecords, Error (line %d): Timerun name missing.\n",
					 COM_GetCurrentParseLine());
			continue;
		}
		if (!timerunTime)
		{
			G_Printf("G_LoadTimerunsRecords, Error (line %d): Timerun time missing.\n",
					 COM_GetCurrentParseLine());
			continue;
		}
		if (!timerunPlayer[0])
		{
			G_Printf("G_LoadTimerunsRecords, Error (line %d): Player name missing.\n",
					 COM_GetCurrentParseLine());
			continue;
		}
		G_DPrintf("G_LoadTimerunsRecords, timerun name: %s, time: %d, player: %s\n",
				  timerunName, timerunTime, timerunPlayer);

		// save it
		// FIXME too slow, need hashing..
		found = qfalse;
		for (i = 0; i < level.numTimeruns && !found; i++)
		{
			if (!strcmp(level.timerunsNames[i], timerunName))
			{
				level.timerunRecordsTimes[i] = timerunTime;
				Q_strncpyz(level.timerunRecordsPlayers[i], timerunPlayer, sizeof(level.timerunRecordsPlayers[i]));
				found = qtrue;
			}
		}

		// no timerun with such name in the level
		if (!found)
			G_Printf("G_LoadTimerunsRecords, Error (line %d): Timerun '%s' is not part of the level.\n",
					 COM_GetCurrentParseLine(), timerunName);
	}
}
...
int G_checkServerToggle(vmCvar_t *cv)
{
	...
	if (cv == &g_nextmap && g_gametype.integer != GT_WOLF_CAMPAIGN)
	{
		if (*cv->string)
		{
