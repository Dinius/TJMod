...
void G_MissileImpact(gentity_t *ent, trace_t *trace, int impactDamage)
{
	...
	// Explosion effect and mark on wall\etc
...
void G_ExplodeMissile(gentity_t *ent)
{
	...
	if (ent->splashDamage)
	{
		...
		if (!tjg_recordMode.integer && !tjg_missilepassthrough.integer)
			trap_Trace(&tr, origin, vec3_origin, vec3_origin, origin, ENTITYNUM_NONE, MASK_SHOT);
		else // Only stops at "solid", aka \care about players etc
			trap_Trace(&tr, origin, vec3_origin, vec3_origin, origin, ENTITYNUM_NONE, CONTENTS_SOLID);
	...
	if (etype == ET_MISSILE || etype == ET_BOMB)
	{
		...
		if (physics.integer == PHYSICS_NORMAL &&
				(
					ent->s.weapon == WP_DYNAMITE || ent->s.weapon == WP_PANZERFAUST || ent->s.weapon == WP_GRENADE_LAUNCHER ||
...
qboolean G_LandmineSnapshotCallback(int entityNum, int clientNum)
{
	...
	if (clEnt->client->sess.playerType == PC_COVERTOPS && clEnt->client->ps.eFlags & EF_ZOOMING && COM_BitCheck(clEnt->client->ps.weapons, WP_BINOCULARS))
	{
		return qtrue;
	}
...
gentity_t *fire_rocket(gentity_t *self, vec3_t start, vec3_t dir)
{
	...
	{
		bolt->s.teamNum = self->client->sess.sessionTeam;
		bolt->owner = self->client->ps.clientNum;
		self->client->hasrockets = 1;
	}
