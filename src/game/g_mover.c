...
void G_TryDoor(gentity_t *ent, gentity_t *other, gentity_t *activator)
{
	...
	if ((ent->s.apos.trType == TR_STATIONARY && ent->s.pos.trType == TR_STATIONARY))
	{
		...
		{
			if (!G_AllowTeamsAllowed(ent, activator))	// door force locked
			{
...
void SP_func_door(gentity_t *ent)
{
	...
	ent->key = -2; // set the key when this ent finishes spawning
...
void SP_func_secret(gentity_t *ent)
{
	...
	ent->key = -1; // set the key when this ent finishes spawning
...
void SP_func_pendulum(gentity_t *ent)
{
	...
	freq = 1 / (M_PI * 2) * sqrt(GAME_GRAVITY / (3 * length));
