...
void G_Script_ScriptLoad(void)
{
	...
	// Dini, mapscript support, "tjmapscripts" per default
	if (tjg_mapScriptDirectory.string[0])
	{
		G_Printf("%s: checking for custom mapscript...\n", GAMEVERSION);
		Q_strncpyz(filename, tjg_mapScriptDirectory.string, sizeof(filename));
		...
		if (len > 0)
		{
			...
			G_Printf("Couldn't find mapscript for %s named %s\n", mapname.string, filename);
	}
...
void G_Script_ScriptParse(gentity_t *ent)
{
	...
	while (1)
	{
		...
		if (!token[0])
		{
			if (!wantName)
			{
				G_Printf("G_Script_ScriptParse(), Error (line %d): '}' expected, end of script found.\n", COM_GetCurrentParseLine());
			}
			break;
		}
		...
		if (token[0] == '}')
		{
			...
			if (wantName)
			{
				G_Printf("G_Script_ScriptParse(), Error (line %d): '}' found, but not expected.\n", COM_GetCurrentParseLine());
				break;
			}
			wantName = qtrue;
		}
		else if (token[0] == '{')
		{
			if (wantName)
			{
				G_Printf("G_Script_ScriptParse(), Error (line %d): '{' found, NAME expected.\n", COM_GetCurrentParseLine());
			}
		}
		else if (wantName)
		{
