/*
 * Contains the code to handle the various commands available with an event
 * script.
 *
 * These functions will return true if the action has been performed and the
 * script should proceed to the next item on the list.
 */
...
qboolean G_ScriptAction_Accum(gentity_t *ent, char *params)
{
	...
	{
		// XXX setup - is this useful at all?
		G_Error("set_to_dynamitecount currently unsupported in tjmod.\n");
...
{
	if (!tjg_EndRound.integer)
	{
		G_Printf("^dTJMOD ^7- WM_ENDROUND: ^zMap ending is disabled!\n");
		return qtrue;
	}
...
qboolean G_ScriptAction_SetRoundTimelimit(gentity_t *ent, char *params)
{
	...
	// Dini, Set the timelimit to 0, aka unlimited (aka sudden death or w\e)
	// TJMOD:BUG, doesnt actually disable ending by time, it just sets it to 0.
	// Endround condition reversed,, endround 0 => no ending of rounds, while 1 = normal behaviour etc..
	if (!tjg_EndRound.integer)
	{
