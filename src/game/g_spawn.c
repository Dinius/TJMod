...
void trigger_push_touch(gentity_t *ent);

void SP_trigger_push_velocity(gentity_t *ent);
void trigger_push_touch_velocity(gentity_t *ent);
...
spawn_t	spawns[] =
{
	...
	// Dini, deemed "obsolete" by ET devs, but heavily used in q3maps.
	// Redirecting to info_notnull, at least most trigger_push from q3 will work then.
	{"target_location", SP_info_notnull}, // WAS SP_target_location
	...
	// Kludge to enable rockets -> panzerfausts etc..
