/*
 * Holds commands that can be executed by the server console, but not remote
 * clients.
 */
...
// Dini, get player name without a caller ID etc
...
void Svcmd_ResetMatch_f(qboolean fDoReset, qboolean fDoRestart)
{
	...
	if (fDoRestart)
	{
		trap_SendConsoleCommand(EXEC_APPEND, va("map_restart 0 %i\n", ((g_gamestate.integer != GS_PLAYING) ? GS_RESET : GS_PLAYING)));
	}
}
...
===================
Nolla - noVoteLimit
XXX setup - convert to a referee command
===================
*/
void Svcmd_noVoteLimit()
{
	gclient_t *cl;
	char name[MAX_TOKEN_CHARS];

	trap_Argv(1, name, sizeof(name));
	if ((cl = G_GetPlayerByName(name)) == NULL)
		return;

	if (cl->sess.noVoteLimit)
		cl->sess.noVoteLimit = qfalse;
	else
	{
		cl->pers.voteCount = 0;
		cl->sess.noVoteLimit = qtrue;
		CPx(cl - level.clients, "cpm \"^7Your vote limit has been removed!\n\"");
	}
}
...
void Svcmd_SetLevel_f(void)
{
	gentity_t	*other;
	int		clientNum, setlvl;
...
#define rc10 va("%c%c%c%c%c%c%c%c%c%c", rc, rc, rc, rc, rc, rc, rc, rc)
	...
	// Random pass for the players level entry..
	...
	if (atoi(arg2) == NULL)
	{
		G_Printf("^1Setlevel:^7 Invalid level specified\n");
		return;
	}
	...
	if (clientNum == -1)
	{
		G_Printf("^1Setlevel:^7 Invalid player specified.\n^3Usage:^7 setlevel <player> <level>\n");
		return;
	}
	other = g_entities + clientNum;

	G_Printf("^1Setlevel:^7 gonna set level %i for %s with name %s \n", setlvl, arg1, other->client->pers.netname);

	filename = "admin.cfg";

	// Already an admin, hence need to rewrite entire config without adding a new one
	if (other->client->pers.admin)
	{
		int i, lvl, curlev;
		lvl = other->client->pers.admin;

		curlev = -99999;

		for (i = 0; i < level.admin.curlev; i++)
		{
			if (!strcmp(level.admin.pass[i], other->client->pers.pass))
			{
				// Would mean the matching already existing admin is number _i_ (0-32768)
				curlev = i;
			}
		}

		/*
		To use pass, cant take the source out of pers.pass.
		Instead, pers.pass must be set at the end, both by sending and recieveing or whatnot.
		*/

		// Update existing admin level
		if (curlev != -99999)
		{
			level.admin.level[curlev] = setlvl;
			Q_strncpyz(level.admin.name[curlev], other->client->pers.netname, sizeof(level.admin.name[curlev]));
			Q_strncpyz(level.admin.pass[curlev], randpass, sizeof(level.admin.pass[curlev]));
			other->client->pers.admin = setlvl;
		}
		// The player is an admin, but it's not been written or saved anywhere...
		// TODO: test safety \ point of having this here,,
		else
		{
			G_Printf("^1Setlevel:^7 Could not find existing admin entry for player, adding new.");
			// Didn't find anyhting to modify, and the player is already an admin, so add it into the config.
			// Note: This might be completely redundant
			level.admin.level[level.admin.curlev] = setlvl;
			Q_strncpyz(level.admin.name[level.admin.curlev], other->client->pers.netname, sizeof(level.admin.name[level.admin.curlev]));
			Q_strncpyz(level.admin.pass[level.admin.curlev], randpass, sizeof(level.admin.pass[level.admin.curlev]));
			other->client->pers.admin = setlvl;
			level.admin.curlev++;
		}
		trap_FS_FOpenFile(filename, &f, FS_WRITE);

		// Write all the admin levels
		...
		for (i = 0; i < level.admin.curlev; i++)
		{
			Com_sprintf(string, sizeof(string),
						...
						"	pass		\"%s\"\n"
						"}\n",
						level.admin.level[i], level.admin.name[i], level.admin.pass[i]);
			trap_FS_Write(string, strlen(string), f);
		}
	...
	{
		// Simply Append it to the config
		...
		Com_sprintf(string, sizeof(string),
					...
					"	pass		\"%s\"\n"
					"}\n",
					setlvl, other->client->pers.netname, randpass);
		Q_strncpyz(level.admin.pass[setlvl], randpass, sizeof(level.admin.pass[setlvl]));
		trap_FS_Write(string, strlen(string), f);
		trap_FS_FCloseFile(f);
	}

	trap_SendServerCommand(other - g_entities, va("setlevel %s", level.admin.pass[setlvl]));
	AP(va("print \"^3Setlevel:^7 %s^7 is now a level %i user!\"", other->client->pers.netname, other->client->pers.admin));
}


void Svcmd_Admintest_f(void)
{
	gentity_t	*other;
	int		clientNum;
	char	arg1[MAX_TOKEN_CHARS];
	...
	if (clientNum == -1)
	{
		G_Printf("^1Admintest:^7 Invalid player specified.\n^3Usage:^7 admintest <player>\n");
		return;
	}
	other = g_entities + clientNum;
	...
	levelname = level.admin.lvl.name[other->client->pers.admin];

	AP(va("print \"^3Admintest:^7 %s^7 %s (level %i)\n\"", other->client->pers.netname, levelname, lvl));
...
qboolean	ConsoleCommand(void)
{
	...
	if (Q_stricmp(cmd, "setlevel") == 0)
	{
		Svcmd_SetLevel_f();
		return qtrue;
	}
	if (Q_stricmp(cmd, "admintest") == 0)
	{
		Svcmd_Admintest_f();
		return qtrue;
	}
