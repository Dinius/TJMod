...
void target_teleporter_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	activator->client->sess.resetspeedontele = qfalse;
	TeleportPlayer(activator, dest->s.origin, dest->s.angles);
}

void target_teleporter_resetspeed_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	gentity_t	*dest;
	...
	activator->client->sess.resetspeedontele = qtrue;
	TeleportPlayer(activator, dest->s.origin, dest->s.angles);
}
...
void SP_target_teleporter(gentity_t *self)
{
	...
	if (self->spawnflags & 1)
	{
		self->use = target_teleporter_resetspeed_use;
	}
	else
	{
...
{
	if ((level.isTimerun && tjg_mapKillEntities.integer) || tjg_mapKillEntities.integer == 2)
	{
		return;
	}
...
void target_starttimer_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	if (self->spawnflags & 1)
	{
		if (VectorLength(client->ps.velocity) > 600)
		{
			...
			CPx(activator - g_entities, "cpm \"^dTJMod^f: ^1Timerun not started, no prejump allowed!\n\"");
			return;
		}
	}
	...
	{
		CPx(activator - g_entities, "cpm \"^dTJMod^f: ^1Timerun not started, invalid playerstate!\n\"");
		return;
	}

	if (client->nextTimerunStartAllowed > client->ps.commandTime)
	{
		CPx(activator - g_entities, "cpm \"^dTJMod^f: ^1Timerun not started, possible teleporter abuse!\n\"");
		return;
	}
	...
	trap_SendServerCommand(activator - g_entities, va("timerun_start %i", timerunNum));

	if (tjg_recordMode.integer && client->pers.tjkey < 1)
		trap_SendServerCommand(activator - g_entities, "warn_notjkey");

	client->startSpeed = sqrt(client->ps.velocity[0] * client->ps.velocity[0] + client->ps.velocity[1] * client->ps.velocity[1]);

	// reset saves (preserve saves done in area with CONTENTS_NOSAVERESET)
	if (client->sess.sessionTeam == TEAM_ALLIES)
		saves = client->sess.alliesSaves;
	else
		saves = client->sess.axisSaves;

	for (i = 0; i < MAX_SAVED_POSITIONS; i++)
	{
		trap_TraceCapsule(&trace, saves[i].origin, activator->r.mins, activator->r.maxs, saves[i].origin, activator->s.number, CONTENTS_NOSAVERESET);

		if (trace.fraction == 1.0f && !tjg_altSL.integer)
			saves[i].valid = qfalse;
	}
	...
	// Send stuff to any spectator currently spectating + starttime
	for (i = 0; i < level.numConnectedClients; i++)
	{
		...
		{
			trap_SendServerCommand(o - g_entities, va("timerun_spec %i %i", timerunNum, client->timerunStartTime + 500));
		}
	}
	...
	if (client->pers.cpmTimes)
		CPx(activator - g_entities, va("cpm \"^d%s^f: ^gStarted!\n\"", client->currentTimerun));
}
...
void SP_target_starttimer(gentity_t *ent)
{
	...
	// Fix for Q3 maps with 99999 wait time and alike @ starttimer
	if (!tjg_timerwait.integer)
		G_SpawnFloat("wait", "0", &ent->wait);
...
void target_stoptimer_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	client->averageSpeed = client->runDistance / (time / 1000);
	...
	switch (type)
	{
		...
		case 1: // best personal
			...
			if (client->sess.logged)
			{
				...
				// Write to log
				if (tjg_recordMode.integer && !g_cheats.integer)
				{
					char	*ip;
					...
					trap_GetUserinfo(activator - g_entities, userinfo, sizeof(userinfo));
					ip = Info_ValueForKey(userinfo, "ip");

					printTJKey = client->pers.tjkey * 2;

					encRecord = va("%.0f%02d%s%.0f%02d%s%03d%s%s%i%i%.0f",
								   client->startSpeed, min, level.rawmapname, client->topSpeed, sec, client->currentTimerun, milli,
								   client->pers.netname, ip, client->pers.tjkey, client->ps.curWeapHeat, client->stopSpeed);

					TJLogPrint("Map: %s, Timerun: %s, Player: %s, Time: %02d:%02d.%03d, Key: %i, %i-%.0f %s%s, %.0f-%.0f %.0f %.0f %s\n",
							   level.rawmapname, client->currentTimerun, client->pers.netname, min, sec, milli, printTJKey,
							   client->ps.curWeapHeat, client->topSpeed, TJMOD_VERSION, ip, client->startSpeed, client->stopSpeed,
							   client->averageSpeed, client->topSpeed, G_SHA1(encRecord));
				}

				// Console print to all players
				AP(va("print \"^fTimerun:^7 %s^7, ^fPlayer:^7 %s^7, ^fTime: ^g%02d:%02d.%03d, ^fJumps: ^z%i, ^fAvg, Top speed: ^z%.0f, %.0f\n\"",
					  client->currentTimerun, client->pers.netname, min, sec, milli, client->ps.curWeapHeat, client->averageSpeed, client->topSpeed));
				...
				// New #1 set, update to local .rec file
				if (tjg_localrecords.integer)
				{
					char		string[1024];
					char		*filename;
					int			i, time, min, sec, milli;

					filename = va("timeruns/%s.rec", level.rawmapname);

					trap_FS_FOpenFile(filename, &level.recFile, FS_WRITE);
					...
					for (i = 0; i < level.numTimeruns; i++)
					{
						if (level.timerunRecordsTimes[i])
						{
							...
							Com_sprintf(string, sizeof(string), "record {\n run_name \"%s\"\n time \"%02d:%02d.%03d\"\n player \"%s\"\n}\n\n",
										level.timerunsNames[i], min, sec, milli, level.timerunRecordsPlayers[i]);

							trap_FS_Write(string, strlen(string), level.recFile);
						}
					}
				...
				CPx(activator - g_entities, "cpm \"You must be logged in for your timeruns to be saved.\n\"");


			trap_SendServerCommand(activator - g_entities, va("runSave %02d-%02d-%03d", min, sec, milli));
			break;
	}
	...
	client->timerunActive = qfalse;
	trap_SendServerCommand(activator - g_entities, va("timerun_stop %i %i", timerunNum, client->sess.timerunLastTime[timerunNum]));
	...
	for (i = 0; i < level.numConnectedClients; i++)
	{
		...
		if (o->client->sess.spectatorClient == activator - g_entities)
			trap_SendServerCommand(o - g_entities, va("timerun_stop %i %i", timerunNum, client->sess.timerunLastTime[timerunNum]));
	}
...
void target_checkpoint_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	trap_SendServerCommand(activator - g_entities, va("timerun_check %i %i %i %i", self->count, delta, time, isFaster));
	// Send stuff to any spectator currently spectating
	for (i = 0; i < level.numConnectedClients; i++)
	{
		o = g_entities + level.sortedClients[i];
		...
		if (o->client->sess.spectatorClient == activator - g_entities)
			trap_SendServerCommand(o - g_entities, va("timerun_check %i %i %i %i", self->count, delta, time, isFaster));
	}
