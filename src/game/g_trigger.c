...
void multi_trigger(gentity_t *ent, gentity_t *activator)
{
	...
	if (ent->wait > 0)
	{
		...
		// FIXME trigger's `random' key doesn't have any function
		if (ent->triggerTime[activator - g_entities] + ent->wait * 1000 > level.time)
			return;

		G_UseTargets(ent, ent->activator);
		ent->triggerTime[activator - g_entities] = level.time;
	}
	// trigger_once or trigger_multiple with wait = -1
	else
	{
		G_UseTargets(ent, ent->activator);
...
/*
 * QUAKED trigger_multiple (.5 .5 .5) ? AXIS_ONLY ALLIED_ONLY NOBOT BOTONLY
 *   SOLDIERONLY LTONLY MEDICONLY ENGINEERONLY COVERTOPSONLY
 *
 * "wait"	Seconds between triggerings, 0 default (0.5 etmain), -1 = one time only.
 * "random"	Wait variance, default is 0.
 *
 * Variable sized repeatable trigger. Must be targeted at one or more
 * entities. The basic time between firing is a random time between
...
void SP_trigger_multiple(gentity_t *ent)
{
	...
	ent->triggerTime = G_Alloc(sizeof(float) * MAX_CLIENTS);
 ...
 *
 * Must be targeted at one or more entities. Once triggered, this entity is
 * destroyed (you can actually do the same thing with trigger_multiple with
 * a wait of -1).
*/
...
Will "keep" player speed and aim speed towards target
=================
*/

void AimAtTargetVelocity(gentity_t *self)
{
	gentity_t	*ent;
	vec3_t		origin;
	float		height, gravity, time, forward;
	...
	// MAKES THE FORWARDGOING HAPPEN, WITHOUT IT = ONLY Z
	// set s.origin2 to the push velocity
	VectorSubtract(ent->s.origin, origin, self->s.origin2);
	self->s.origin2[2] = 0;
	dist = VectorNormalize(self->s.origin2);
	...
	if (other->client)
	{
		...
		xyspeed = sqrt(other->client->ps.velocity[0] * other->client->ps.velocity[0] +  other->client->ps.velocity[1] * other->client->ps.velocity[1]);

		diff = xyspeed - forward;
		diff = sqrt(diff * diff);
		forward += xyspeed;
	}
	...
	// Makes the Z height happen
	self->s.origin2[2] = time * gravity + sqrt(fabs((diff * (0.5f * gravity))));
}
...
{
	return; // Disabling this.. CBA to make a proper one from scratch or rewrite wsw one. Screw it.
	InitTrigger(self);
	...
	self->think = AimAtTargetVelocity;
	self->nextthink = level.time + FRAMETIME;
	trap_LinkEntity(self);
}
 ...
 *
 * Allows client side prediction of teleportation events. Must point at
 * a target_position, which will be the teleport destination.
 */
...
{
	save_position_t	*saves[] = {other->client->sess.alliesSaves, other->client->sess.axisSaves};
	int i, j;
	...
	if (!tjg_trigger_savereset.integer)
		return;
	...
	CPx(other - g_entities, "cpm \"^dTJMod^f:^7 Your saves were removed.\n\"");
}
...
void savereset_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	if (self->touch)
	{
		self->touch = NULL;
	...
	self->think = savereset_think;
}

void SP_trigger_savereset(gentity_t *self)
{
	InitTrigger(self);
 ...
 *
 * Player must be carrying the proper flag for it to trigger. It will call
 * the "death" function in the object's script.
 */
 ...
 *   MESSAGE_OVERRIDE TANK IS_OBJECTIVE IS_HEALTHAMMOCABINET IS_COMMANDPOST
 *
