/*
 * Shared definitions between all ET parts. Included first by ALL program
 * modules. A user mod should never modify this file.
 *
 * Setup - since there will be no further SDK update I think it's save to
 * modify this file a bit, so I added documentation and done some cleanups.
 */
...
extern	vec4_t		colorD;
extern	vec4_t		colorF;
...
#define gethex(ch) ((ch) > '9' ? ((ch) >= 'a' ? ((ch) - 'a' + 10) : ((ch) - 'A' + 10)) : ((ch) - '0'))

// check if it's format rrggbb r,g,b e {0..9} U {A...F}
#define Q_IsHexColorString(p) (isxdigit(*(p)) && isxdigit(*((p)+1)) && isxdigit(*((p)+2)) && isxdigit(*((p)+3)) && isxdigit(*((p)+4)) && isxdigit(*((p)+5)))
...
/*
 * Information needed by both the client and server to predict player motion
 * and actions, nothing outside of pmove should modify it, or some degree of
 * prediction error will occur.
 *
 * You can't add anything to this without modifying the code in msg.c.
 * (Unless it doesn't need transmitted over the network, in which case it
 * should probably go in the pmext struct anyway.)
 *
 * playerState_t is a full superset of entityState_t as it is used by players,
 * so if a playerState_t is transmitted, the entityState_t can be fully
 * derived from it.
 *
 * Note: All fields in here must be 32 bits (or those within sub-structures)
 * but not everytime complete 32 bits are transmitted!
 */
typedef struct playerState_s
{
	/// Contains cmd->serverTime of last executed command.
	int			commandTime;
	...
	int			stats[MAX_STATS];			// 16bit values
	int			persistant[MAX_PERSISTANT];	// stats that aren't cleared on death
	int			powerups[MAX_POWERUPS];		// level.time that the powerup runs out (32bit values)
