...
/* =========== UI_LoadMods CHRUKER: b090 - Sorting the mods list =========== */
				...
				case SORT_FILTERS:
					...
					else
					{
						*numhandles = 7;
						...
						if (needpass) handles[0] = uiInfo.passwordFilter;
						else handles[0] = -1;
						if (friendlyfire) handles[1] = uiInfo.friendlyFireFilter;
						else handles[1] = -1;
						if (maxlives) handles[2] = uiInfo.maxLivesFilter;
						else handles[2] = -1;
						if (punkbuster) handles[3] = uiInfo.punkBusterFilter;
						else handles[3] = -1;
						if (weaponrestrictions < 100) handles[4] = uiInfo.weaponRestrictionsFilter;
						else handles[4] = -1;
						if (antilag) handles[5] = uiInfo.antiLagFilter;
						else handles[5] = -1;
						if (balancedteams) handles[6] = uiInfo.teamBalanceFilter;
						else handles[6] = -1;
...
cvarTable_t		cvarTable[] =
{
	...
	{ NULL, "cg_drawoldCompass", "0", CVAR_ARCHIVE },
	{ NULL, "cg_drawRoundTimer", "1", CVAR_ARCHIVE },
	...
	{ NULL, "bot_enable", "0", CVAR_ARCHIVE },
	...
	{ NULL, "tjrefereePassword", "none", CVAR_ARCHIVE },
	...
	{ NULL,	"server_motd0", "^7Enemy Territory", CVAR_ARCHIVE },
	{ NULL,	"server_motd1", "", CVAR_ARCHIVE },
	{ NULL,	"server_motd2", "^dTJMod Server", CVAR_ARCHIVE },
	{ NULL,	"server_motd3", "", CVAR_ARCHIVE },
	{ NULL,	"server_motd4", "", CVAR_ARCHIVE },
	{ NULL,	"server_motd5", "^zwww.^7Trickjump^z.me", CVAR_ARCHIVE },
	...
	for (i = 0, cv = cvarTable ; i < cvarTableSize ; i++, cv++)
	{
		if (cv->vmCvar)
		{
			...
			if (cv->modificationCount != cv->vmCvar->modificationCount)
			{
				...
				{
					BG_setColor(cg_crosshairColor.string, uiInfo.xhairColor, cg_crosshairAlpha.value, "cg_crosshairColor");
				}

				if (cv->vmCvar == &cg_crosshairColorAlt || cv->vmCvar == &cg_crosshairAlphaAlt)
				{
					BG_setColor(cg_crosshairColorAlt.string, uiInfo.xhairColorAlt, cg_crosshairAlphaAlt.value, "cg_crosshairColorAlt");
				}
			}
		}
	}
